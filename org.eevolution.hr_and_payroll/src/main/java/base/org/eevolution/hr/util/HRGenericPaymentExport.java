 ...
 */
public class HRGenericPaymentExport implements HRPaymentExport
{
	/** Logger								*/
	static private CLogger	s_log = CLogger.getCLogger (HRGenericPaymentExport.class);
	...
	public int exportToFile (List<MHRPaySelectionCheck> checks, File file, StringBuffer err)
	{
		...
		try
		{
			...
			for (MHRPaySelectionCheck mpp : checks)
			{
				...
				{
					MHRMovement movement = new MHRMovement(psl.getCtx(), psl.getHR_Movement_ID(), psl.get_TrxName());
					MHRConcept concept = new MHRConcept(psl.getCtx(), movement.getHR_Concept_ID(), psl.get_TrxName());
					comment.append(concept.getName());
				}
