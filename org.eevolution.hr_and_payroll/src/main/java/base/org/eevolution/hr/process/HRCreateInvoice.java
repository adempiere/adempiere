...
public class HRCreateInvoice extends HRCreateInvoiceAbstract {
    ...
    protected String doIt() throws Exception {
        ...
        AtomicReference<Timestamp> dateInvoice = new AtomicReference<Timestamp>(getDateInvoiced());
        if(dateInvoice.get() == null && process.getHR_Period_ID() > 0) {
        	MHRPeriod period = MHRPeriod.getById(getCtx(), process.getHR_Period_ID(), get_TrxName());
                    ...
                    Trx.run(trxName -> {
                        ...
                        for (MHRMovement movement : getPayrollMovement(getHRProcessId(), partnerId, trxName)) {
                            ...
                            if (attribute != null && attribute.getC_DocType_ID() == 0) {
                            	MHRConcept concept = MHRConcept.getById(getCtx(), movement.getHR_Concept_ID(), trxName);
                                log.log(Level.SEVERE, "@HR_Employee_ID@ " + partner.getName() + "  @HR_Concept_ID@  " + concept.getName() + " @C_DocType_ID@ @NotFound@ : ");
                                addLog(0, null, null, "@HR_Employee_ID@ " + partner.getName() + "  @HR_Concept_ID@  " + concept.getName() + " @C_DocType_ID@ @NotFound@ : ");
                                continue;
                            }

                            if (attribute != null && attribute.getC_Charge_ID() == 0) {
                            	MHRConcept concept = MHRConcept.getById(getCtx(), movement.getHR_Concept_ID(), trxName);
                                log.log(Level.SEVERE, "@HR_Employee_ID@ " + partner.getName() + "  @HR_Concept_ID@  " + concept.getName() + " @C_Charge_ID@ @NotFound@ : ");
                                addLog(0, null, null, "@HR_Employee_ID@ " + partner.getName() + "  @HR_Concept_ID@  " + concept.getName() + " @C_Charge_ID@ @NotFound@ : ");
                                continue;
                            }
                            ...
                            if (movement.getC_BPartner_ID() != lastPartnerId) {
                                ...
                                invoice = createInvoice(process, partner, employee, attribute.getC_DocType_ID(), dateInvoice.get());
                                if (invoice == null)
                                    continue;
    ...
    private MInvoiceLine createInvoiceLine(MInvoice invoice, MHREmployee employee, MHRMovement movement, int chargeId) {
        ...
        MHRConcept concept = MHRConcept.getById(getCtx(), movement.getHR_Concept_ID(), movement.get_TrxName());
        ...
        invoiceLine.setTax();
        invoiceLine.saveEx();
        addLog(invoiceLine.getLine(), invoice.getDateInvoiced(), invoiceLine.getLineNetAmt(), concept.getName());
