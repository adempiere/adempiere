...
public class GenerateInOutBound extends SvrProcess {
	...
	protected String doIt() throws Exception {
		...
		if (p_C_DocType_ID > 0) {
			...
			int C_DocType_ID = MDocType
					.getDocType(MDocType.DOCBASETYPE_WarehouseManagementOrder);
			if (C_DocType_ID <= 0) {
				throw new DocTypeNotFoundException(
		...
		Collection<MOrderLine> olines = new Query(getCtx(),
				I_C_OrderLine.Table_Name, whereClause, get_TrxName())
				.setClient_ID()
		...
		for (MOrderLine oline : olines) {
			...
			boundline.setM_AttributeSetInstance_ID(oline
					.getM_AttributeSetInstance_ID());
			boundline.setMovementQty(oline.getQtyOrdered().subtract(
