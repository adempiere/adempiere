...
public class InvoiceManager extends AbstractDocumentManager
{
    ...
    public static MInvoice createVendorInvoice(Properties ctx, int purchaseOrderId,Integer[] purchaseOrderLineIds,String trxName) throws OperationException
    {
        ...
        if(purchaseOrder.getOrderType().equals(UDIOrderTypes.POS_GOODS_RETURN_NOTE.getOrderType())) 
            ...
            for( int i=0;i < invoiceLine.length;i++)
            {
                ...
                udiInvoiceLine = new UDIMInvoiceLine(invoiceLine[i]);
                udiInvoiceLine.save();
            }
    ...
    public static MInvoice completeInvoice(Properties ctx, MInvoice invoice) throws OperationException
    {
        ...
        udiInvoice.processIt(DocumentEngine.ACTION_Complete);
        
        return udiInvoice.getInvoice();
    }
    ...
    private static void updateVendorInvoice(Properties ctx,MInvoice invoice, String trxName) throws OperationException
    {
    	...
    	UDIMInvoiceLine udiVInvoiceLine;
    	...
    	for(int i=0;i<CInvoiceLine.length;i++)
    	{
    		...
    		//Fixed
    		udiVInvoiceLine = new UDIMInvoiceLine(VInvoiceLine[i]);
    		udiVInvoiceLine.save();
    	}
    }
    ...
    public static MInvoice getCounterInvoice(Properties ctx,MInvoice customerInvoice) throws OperationException
    {
        ...
        if (vendorInvoice.getC_DocType_ID() == 0)
        {
            ...
            UDIMInvoice udiInvoice = new UDIMInvoice(vendorInvoice);
            udiInvoice.save();
...
//    public static WebDocumentBean changeInvoiceStatus(Properties ctx, MInvoice invoice, ChangeDocumentStatusBean bean, String trxName) throws OperationException, DataException
//    {
//        WebDocumentBean invoiceBean = new WebDocumentBean();
//        
//        String submit = bean.getSubmit();
//        
//        if (invoice == null)
//        {
//            throw new OperationException("Cannot change the status on an invoice that does not exist anymore.");
//        }
//       
//        if (submit.equals(Constants.INVOICE_ACTION_COMPLETE))
//        {
//            invoice = InvoiceManager.completeInvoice(ctx, invoice.get_ID(), trxName);
//            MInvoice counterInvoice = getCounterInvoice(ctx, invoice.get_ID(), trxName);
//            //complete counter invoice also
//            InvoiceManager.completeInvoice(ctx, counterInvoice.get_ID(), trxName);
//            if(invoice.isSOTrx()==true)
//            try 
//            {
//                MInvoiceLine invoiceLines[]=invoice.getLines();
//                MessageManager.sendInvoiceMessage(invoiceLines[0],UDIDocActionValues.CONFIRM);
//            } 
...
//        }
//        
//        if (submit.equals(Constants.INVOICE_ACTION_CANCEL))
//        {
//            invoice = voidInvoice(ctx, invoice);
//            
//            try 
//            {
//                MInvoiceLine invoiceLines[]=invoice.getLines();
//                MessageManager.sendInvoiceMessage(invoiceLines[0],UDIDocActionValues.CANCEL);
//            } 
//            catch (InvalidEmailException e) 
//            {
//                throw new OperationException(e.getMessage());
...
//        }
//        
//        if (submit.equals(Constants.INVOICE_ACTION_DELETE))
//        {
//            invoice = deleteInvoice(ctx, invoice);
//        }
//        
//        if (submit.equals(Constants.INVOICE_ACTION_PAID))
//        {
//            MPayment apPayment=PaymentManager.generatePayments(ctx, invoice);
//
...
//        }
//        
//        if (submit.equals(Constants.INVOICE_ACTION_PAID_AND_SHIP))
//        {
//            PaymentManager.paidAndShip(ctx, invoice);
//            invoice = new MInvoice(ctx, invoice.get_ID(), invoice.get_TrxName());
//        }
//     
//        invoiceBean = InvoiceManager.getWebInvoiceBean(invoice);
//        
//        return invoiceBean;
//    }
//    
 ...
 /*   private static MInvoice createAndCompleteCustomerInvoiceForOrder(Properties ctx, int salesOrderId, String trxName) throws OperationException
    {
        MOrder salesOrder = new MOrder(ctx,salesOrderId,trxName);
        MInvoice invoice = null;
        try
        {
            ...
            UDIMInvoice udiCInvoice= new UDIMInvoice(invoice);
            udiCInvoice.processIt(DocumentEngine.ACTION_Complete);
 ...
 */   
    ...
    public static ArrayList<WebOrderLineBean> populateInvoiceLines(Properties ctx, MInvoice invoice,boolean abbr) throws  OperationException
    {
        ...
        for (int i = 0; i < lines.length; i++)
        {
            ...
            BigDecimal roundedLineNetAmount = POSManager.round(line.getLineNetAmt(), 2);
            ...
            BigDecimal roundedTaxAmount = POSManager.round(lineTaxAmount, 2);
            orderLineBean.setTaxAmt(roundedTaxAmount);
            BigDecimal lineTotalAmount = line.getLineNetAmt().add(orderLineBean.getTaxAmt());
            BigDecimal roundedLineTotalAmount = POSManager.round(lineTotalAmount, 2);
            ...
            qty=qty+line.getQtyEntered().intValue();
            orderLineBean.setQtyTotal(new BigDecimal(qty)) ;
