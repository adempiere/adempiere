...
<tr>
	<td width="50%" valign="top">
		...
		<div>
			...
			<pos:element columnName="PaymentRule"/>:	<bean:write name="<%=Constants.WEB_DOCUMENT_HEADER_BEAN%>" property="paymentType"/>
			</logic:present>		
		
			(
			<logic:present name="<%=Constants.PAYMENT_BY_CASH%>">
				<pos:element columnName="cash"/>: <bean:write name="<%=Constants.PAYMENT_BY_CASH%>"/>
			</logic:present>
					
			<logic:present name="<%=Constants.PAYMENT_BY_CARD%>">
				, <pos:element columnName="card"/>:	<bean:write name="<%=Constants.PAYMENT_BY_CARD%>"/>
			</logic:present>
			
			<logic:present name="<%=Constants.PAYMENT_BY_CHEQUE%>">
				, <pos:element columnName="cheque"/>:	<bean:write name="<%=Constants.PAYMENT_BY_CHEQUE%>"/>
			</logic:present>
			)
		</div>
	...
	<td width="50%" valign="top">
		...
		<div align="right">
			<logic:present name="<%=Constants.SALES_REP%>">
			  	<pos:element columnName="SalesRep_Name"/>:<bean:write name="<%=Constants.SALES_REP%>"/>
			</logic:present>
		</div>
		...
		<div align="right">
			<pos:element columnName="Remark"/>: <bean:write name="<%=Constants.DESCRIPTION%>"/>
		</div>
		</logic:present>
