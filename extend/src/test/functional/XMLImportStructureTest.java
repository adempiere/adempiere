...
public class XMLImportStructureTest extends TestCase {
	...
	// NO NEED TO CHANGE fileName_DefaultValue
	private String fileName_DefaultValue = "test.properties";

	private String fileName_Key = "PackOut";
	...
	protected void setUp() throws Exception {
		...
		// CHANGE THIS ACCORDING TO THE PATH OF THE ADEMPIERE.PROPERTIES FILE
		String testProp = "/Users/fernando/Documents/workspace/adempiere_trunk/adempiere/Adempiere/Adempiere.properties";

		testProperties = new Properties();
		testProperties.load(new FileInputStream(testProp));
		fileName_Value = testProperties.getProperty(fileName_Key,
				fileName_DefaultValue);
		isClient_Value = "Y".equals(testProperties.getProperty(isClient_Key,
				isClient_DefaultValue));
		AD_User_ID_Value = Integer.parseInt(testProperties.getProperty(
				AD_User_ID_Key, AD_User_ID_DefaultValue));
		AD_Client_ID_Value = Integer.parseInt(testProperties.getProperty(
				AD_Client_ID_Key, AD_Client_ID_DefaultValue));

		m_Ctx = new Properties();
		m_Ctx.setProperty("#AD_User_ID", new Integer(AD_User_ID_Value)
				.toString());
		m_Ctx.setProperty("#AD_Client_ID", new Integer(AD_Client_ID_Value)
				.toString());

		System.out.println("m_Ctx: " + m_Ctx);

		if (fileName_Value.length() < 1) {
			assertEquals("Please specify path to Adempiere.properties file!",
					true, false);
		}
		...
		// if (args.length >= 6) {
		// CConnection cc = CConnection.get(Database.DB_ORACLE, args[1],
		// Integer.valueOf(args[2]).intValue(), args[3], args[4], args[5]);
		...
		/*
		 * Available levels: Level.OFF, Level.SEVERE, Level.WARNING, Level.INFO,
	...
	public class TestPackInHandler extends DefaultHandler {
		...
		public void startElement(String uri, String localName, String qName,
			...
			/*
			 * System.out.println("In startElement"); System.out.println("uri: " +
			 * uri); System.out.println("localName: " + localName);
			...
			if (atts != null) {
				...
				List<Map<String, String>> m_attsMapList = elementAttsMap
						.get(qName);
				if (m_attsMapList == null)
					elementAttsMap
							.put(
									qName,
									m_attsMapList = new ArrayList<Map<String, String>>());
				// System.out.println("adding atts.getLength: " +
				// atts.getLength());
				// System.out.println("atts.getClass().getName(): " +
				// atts.getClass().getName());
		...
		/*
		 * The XML elements that need to be processed are as follows: table
		 * referencelist printformatitem form message workflowNode column
		 * printformat menuset workflowNodeNext referencetable processpara
		 * dynvalrule process preference tab field workflow menu window
		 * reference
		 * 
		 * Dependency Analysis:
		 * 
		 * windows don't depend on anything dynamic validation rules don't
		 * depend on anything referencelist don't depend on anything forms don't
		 * depend on anything messages don't depend on anything references don't
		 * depend on anything
		 * 
		 * tables depend on validation rules, windows columns depend on
		 * processes, references, tables, dynamic validation rules Fields depend
		 * on tables, windows printformats depend on tables, other printformats
		 * printformatitems depend on printformats, tables workflows depend on
		 * tables processes depend on workflows, printformats, reportviews
		 * workflowNodes depend on workflows, windows, processes, forms menus
		 * depend on windows, processes, forms, workflows, other menus
		 * workflowNodeNexts depend on workflows, workflowNodes referencetables
		 * depend on tables, columns processparas depend on processes,
		 * references, dynamic validation rules preferences depend on windows
		...
		public void handleWindowsImport() {
			...
			if (elementAttsMap.containsKey("window")) {
				...
				List<Map<String, String>> m_windowsAttsMapList = elementAttsMap
						.get("window");
				System.out.println("processing " + m_windowsAttsMapList.size()
						+ " windows");
				while (m_windowsAttsMapList.size() > 0) {
					Map<String, String> attsMap = m_windowsAttsMapList.get(0);
					System.out.println("window name: "
							+ attsMap.get("ADWindowNameID"));
		...
		public void handleDynValRulesImport() {
			...
			if (elementAttsMap.containsKey("dynvalrule")) {
				...
				List<Map<String, String>> m_dynvalrulesAttsMapList = elementAttsMap
						.get("dynvalrule");
				System.out.println("processing "
						+ m_dynvalrulesAttsMapList.size()
						+ " dynamic validation rules");
				while (m_dynvalrulesAttsMapList.size() > 0) {
					Map<String, String> attsMap = m_dynvalrulesAttsMapList
							.get(0);
					System.out.println("dynamic validation rule name: "
							+ attsMap.get("Name"));
		...
		public void handleReferenceListsImport() {
			...
			if (elementAttsMap.containsKey("referencelist")) {
				...
				List<Map<String, String>> m_referenceListsAttsMapList = elementAttsMap
						.get("referencelist");
				System.out.println("processing "
						+ m_referenceListsAttsMapList.size()
						+ " referencelists");
				int i = 0;
				while (m_referenceListsAttsMapList.size() > 0) {
					Map<String, String> attsMap = m_referenceListsAttsMapList
							.get(0);
					System.out.println("referenceList num: " + i++ + " name: "
							+ attsMap.get("Name"));
		...
		public void handleFormsImport() {
			...
			if (elementAttsMap.containsKey("form")) {
				...
				List<Map<String, String>> m_formsAttsMapList = elementAttsMap
						.get("form");
				System.out.println("processing " + m_formsAttsMapList.size()
						+ " forms");
				while (m_formsAttsMapList.size() > 0) {
					Map<String, String> attsMap = m_formsAttsMapList.get(0);
					System.out.println("form name: "
							+ attsMap.get("ADFormNameID"));
					importForm(attsMap);
					attsMap.clear();
		...
		public void handleMessagesImport() {
			...
			if (elementAttsMap.containsKey("message")) {
				...
				List<Map<String, String>> m_messagesAttsMapList = elementAttsMap
						.get("message");
				System.out.println("processing " + m_messagesAttsMapList.size()
						+ " messages");
				while (m_messagesAttsMapList.size() > 0) {
					Map<String, String> attsMap = m_messagesAttsMapList.get(0);
					System.out
							.println("message value: " + attsMap.get("Value"));
		...
		public void handleReferencesImport() {
			...
			if (elementAttsMap.containsKey("reference")) {
				...
				List<Map<String, String>> m_referencesAttsMapList = elementAttsMap
						.get("reference");
				System.out.println("processing "
						+ m_referencesAttsMapList.size() + " references");
				int i = 0;
				while (m_referencesAttsMapList.size() > 0) {
					Map<String, String> attsMap = m_referencesAttsMapList
							.get(0);
					System.out.println("reference num: " + i++ + " name: "
							+ attsMap.get("Name"));
		...
		public void handleTablesImport() {
			...
			if (elementAttsMap.containsKey("table")) {
				...
				List<Map<String, String>> m_tablesAttsMapList = elementAttsMap
						.get("table");
				System.out.println("processing " + m_tablesAttsMapList.size()
						+ " tables");
		...
		public void handleColumnsImport() {
			...
			if (elementAttsMap.containsKey("column")) {
				...
				List<Map<String, String>> m_columnsAttsMapList = elementAttsMap
						.get("column");
				System.out.println("processing " + m_columnsAttsMapList.size()
						+ " columns");
				int i = 0;
				while (m_columnsAttsMapList.size() > 0) {
					Map<String, String> attsMap = m_columnsAttsMapList.get(0);
					// if(attsMap.get("ADColumnNameID").equals("isAvailable") &&
					// attsMap.get("ADTableNameID").equals("S_Resource")) {
					System.out.println("column num: " + i++ + " name: "
							+ attsMap.get("ADColumnNameID"));
					System.out.println("table name: "
							+ attsMap.get("ADTableNameID"));
		...
		public void handleFieldsImport() {
			...
			if (elementAttsMap.containsKey("field")) {
				...
				List<Map<String, String>> m_fieldsAttsMapList = elementAttsMap
						.get("field");
				System.out.println("processing " + m_fieldsAttsMapList.size()
						+ " fields");
				int i = 0;
				while (m_fieldsAttsMapList.size() > 0) {
					Map<String, String> attsMap = m_fieldsAttsMapList.get(0);
					System.out.println("field num: " + i++ + " name: "
							+ attsMap.get("ADFieldNameID"));
					importField(attsMap);
					attsMap.clear();
		...
		public void handlePrintFormatsImport() {
			...
			if (elementAttsMap.containsKey("printformat")) {
				...
				List<Map<String, String>> m_printformatsAttsMapList = elementAttsMap
						.get("printformat");
				System.out.println("processing "
						+ m_printformatsAttsMapList.size() + " printformats");
				while (m_printformatsAttsMapList.size() > 0) {
					Map<String, String> attsMap = m_printformatsAttsMapList
							.get(0);
					System.out.println("printformat name: "
							+ attsMap.get("Name"));
		...
		public void handlePrintFormatItemsImport() {
			...
			if (elementAttsMap.containsKey("printformatitem")) {
				...
				List<Map<String, String>> m_printformatitemsAttsMapList = elementAttsMap
						.get("printformatitem");
				System.out.println("processing "
						+ m_printformatitemsAttsMapList.size()
						+ " printformats");
				while (m_printformatitemsAttsMapList.size() > 0) {
					Map<String, String> attsMap = m_printformatitemsAttsMapList
							.get(0);
					System.out.println("printformatitem name: "
							+ attsMap.get("Name"));
		...
		public void handleWorkflowsImport() {
			...
			if (elementAttsMap.containsKey("workflow")) {
				...
				List<Map<String, String>> m_workflowsAttsMapList = elementAttsMap
						.get("workflow");
				System.out.println("processing "
						+ m_workflowsAttsMapList.size() + " workflows");
		...
		public void handleProcessesImport() {
			...
			if (elementAttsMap.containsKey("process")) {
				...
				List<Map<String, String>> m_processesAttsMapList = elementAttsMap
						.get("process");
				System.out.println("processing "
						+ m_processesAttsMapList.size() + " processes");
		...
		public void handleWorkflowNodesImport() {
			...
			if (elementAttsMap.containsKey("workflowNode")) {
				...
				List<Map<String, String>> m_workflowNodesAttsMapList = elementAttsMap
						.get("workflowNode");
				System.out.println("processing "
						+ m_workflowNodesAttsMapList.size() + " workflowNodes");
				int i = 0;
				while (m_workflowNodesAttsMapList.size() > 0) {
					Map<String, String> attsMap = m_workflowNodesAttsMapList
							.get(0);
					System.out.println("worklfowNode  num: " + i++ + " name: "
							+ attsMap.get("Name"));
		...
		public void importForm(Map<String, String> attsMap) {
			...
			if (entitytype.compareTo("U") == 0
					|| entitytype.compareTo("D") == 0) {
				...
				m_Form.setIsBetaFunctionality(Boolean.valueOf(
						attsMap.get("isBetaFunctionality")).booleanValue());
				m_Form.setAccessLevel(attsMap.get("AccessLevel"));
				m_Form.setDescription(attsMap.get("Description").replaceAll(
						"'", "''").replaceAll(",", ""));
				m_Form.setEntityType(attsMap.get("EntityType"));
				m_Form.setHelp(attsMap.get("Help").replaceAll(",", ""));
				m_Form
						.setIsActive(attsMap.get("isActive") != null ? Boolean
								.valueOf(attsMap.get("isActive"))
								.booleanValue() : true);
				...
				try {
					if (m_Form.save(m_trxName) == true) {
						...
						record_log(1, m_Form.getName(), "Form",
								m_Form.get_ID(), AD_Backup_ID, Object_Status,
								"AD_Form", get_IDWithColumn("AD_Table",
										"TableName", "AD_Form"));
					} else {
						System.out.println("m_Form.save failed");
						record_log(0, m_Form.getName(), "Form",
								m_Form.get_ID(), AD_Backup_ID, Object_Status,
								"AD_Form", get_IDWithColumn("AD_Table",
										"TableName", "AD_Form"));
					}
				} catch (SAXException e) {
					System.out.println("Exception in importForm: "
							+ e.getMessage());
		...
		public void importColumn(Map<String, String> attsMap) {
			...
			if (entitytype.compareTo("U") == 0
				...
				int tableid = get_IDWithColumn("AD_Table", "TableName", attsMap
						.get("ADTableNameID"));
				int id = get_IDWithMasterAndColumn("AD_Column", "ColumnName",
				...
				/**
				 * //Adjust for difference between Oracle and PostgreSql
				 * DataTypes if (m_DatabaseType.equals("Oracle")){ if
				 * (Name.equals("TIMESTAMP")) Name = "Date"; else if
				 * (Name.equals("TIMESTAMP+Time")) Name = "Date+Time"; else if
				 * (Name.equals("NUMERIC")) Name = "Number";} else if
				 * (m_DatabaseType.equals("Sybase")){ if
				 * (Name.equals("TIMESTAMP")) Name = "Date"; else if
				 * (Name.equals("TIMESTAMP+Time")) Name = "Date+Time"; else if
				 * (Name.equals("NUMERIC")) Name = "Number";} else if
				 * (m_DatabaseType.equals("PostgreSQL")){ if
				 * (Name.equals("Date")) Name = "TIMESTAMP"; else if
				 * (Name.equals("Date+Time")) Name = "TIMESTAMP+Time"; else if
				 * (Name.equals("Number")) Name = "NUMERIC";}
				 */
				...
				m_Column.setDescription(attsMap.get("Description").replaceAll(
						"'", "''").replaceAll(",", ""));
				m_Column.setEntityType(attsMap.get("EntityType"));

				if (Integer.parseInt(attsMap.get("FieldLength")) > 0)
					m_Column.setFieldLength(Integer.parseInt(attsMap
							.get("FieldLength")));
				m_Column.setHelp(attsMap.get("Help").replaceAll("'", "''")
						.replaceAll(",", ""));
				m_Column
						.setIsActive(attsMap.get("isActive") != null ? Boolean
								.valueOf(attsMap.get("isActive"))
								.booleanValue() : true);
				m_Column.setIsAlwaysUpdateable((Boolean.valueOf(attsMap
						.get("isAlwaysUpdateable")).booleanValue()));
				// m_Column.setIsEncrypted(atts.getValue("isEncrypted"));
				m_Column.setIsIdentifier((Boolean.valueOf(attsMap
						.get("isIdentifier")).booleanValue()));
				m_Column.setIsKey((Boolean.valueOf(attsMap.get("isKey"))
						.booleanValue()));
				m_Column.setIsMandatory((Boolean.valueOf(attsMap
						.get("isMandatory")).booleanValue()));

				m_Column.setIsParent((Boolean.valueOf(attsMap.get("isParent"))
						.booleanValue()));
				m_Column.setIsSelectionColumn((Boolean.valueOf(attsMap
						.get("isSelectionColumn")).booleanValue()));
				m_Column.setIsSyncDatabase(attsMap.get("getIsSyncDatabase"));

				m_Column.setIsTranslated((Boolean.valueOf(attsMap
						.get("isTranslated")).booleanValue()));
				m_Column.setIsUpdateable((Boolean.valueOf(attsMap
						.get("isUpdateable")).booleanValue()));
				...
				// Setup Element.
				id = get_IDWithColumn("AD_Element", "ColumnName", m_Column
						.getColumnName());
				...
				if (element.getAD_Element_ID() == 0) {
					...
					try {
						if (element.save(m_trxName) == true) {
							...
							record_log(1, m_Column.getName(), "Element",
									element.getAD_Element_ID(), AD_Backup_ID,
									"New", "AD_Element", get_IDWithColumn(
											"AD_Table", "TableName",
											"AD_Element"));
						} else {
							System.out.println("element.savefailed");
							record_log(0, m_Column.getName(), "Element",
									element.getAD_Element_ID(), AD_Backup_ID,
									"New", "AD_Element", get_IDWithColumn(
											"AD_Table", "TableName",
											"AD_Element"));
						...
						System.out
								.println("import Column element.save exception: "
										+ e.getMessage());
				...
				// Don't create columns by default, just if
				// getIsSyncDatabase='Y'
				if (recreateColumn) {
				...
				try {
					...
					if (m_Column.save(m_trxName) == true) {
						...
						record_log(1, m_Column.getName(), "Column", m_Column
								.get_ID(), AD_Backup_ID, Object_Status,
								"AD_Column", get_IDWithColumn("AD_Table",
										"TableName", "AD_Column"));
					} else {
						System.out.println("m_Column.save failed");
						record_log(0, m_Column.getName(), "Column", m_Column
								.get_ID(), AD_Backup_ID, Object_Status,
								"AD_Column", get_IDWithColumn("AD_Table",
										"TableName", "AD_Column"));
					}
					...
					System.out
							.println("Exception in importColumn with m_Column.save: "
									+ e.getMessage());
				...
				if (attsMap.get("ADTableNameID").equals("PP_WF_Node_Asset")
						|| attsMap.get("ADTableNameID").equals("AD_User"))
					recreateColumn = false;
				else
					recreateColumn = true;
				...
				if (recreateColumn) {
					...
					try {
						...
						if (success == 1) {
							record_log(1, m_Column.getColumnName(), "dbColumn",
									m_Column.get_ID(), 0, Object_Status,
									attsMap.get("ADTableNameID").toUpperCase(),
									get_IDWithColumn("AD_Table", "TableName",
											attsMap.get("ADTableNameID")
													.toUpperCase()));
						} else {
							record_log(0, m_Column.getColumnName(), "dbColumn",
									m_Column.get_ID(), 0, Object_Status,
									attsMap.get("ADTableNameID").toUpperCase(),
									get_IDWithColumn("AD_Table", "TableName",
											attsMap.get("ADTableNameID")
						...
						System.out.println("Exception in importColumn: "
								+ e.getMessage());
					}
				}
			}
		...
		public void importTable(Map<String, String> attsMap) {
			...
			if (entitytype.equals("U") || entitytype.equals("D")) {
				...
				m_Table.setDescription(attsMap.get("Description").replaceAll(
						"'", "''").replaceAll(",", ""));
				m_Table.setEntityType(attsMap.get("EntityType"));
				m_Table.setHelp(attsMap.get("Help").replaceAll("'", "''")
						.replaceAll(",", ""));
				m_Table
						.setIsActive(attsMap.get("isActive") != null ? Boolean
								.valueOf(attsMap.get("isActive"))
				...
				m_Table.setIsChangeLog(Boolean.valueOf(
						attsMap.get("isChangeLog")).booleanValue());
				m_Table.setIsDeleteable(Boolean.valueOf(
						attsMap.get("isDeleteable")).booleanValue());
				m_Table.setIsHighVolume(Boolean.valueOf(
						attsMap.get("isHighVolume")).booleanValue());
				m_Table.setIsSecurityEnabled(Boolean.valueOf(
						attsMap.get("isSecurityEnabled")).booleanValue());
				m_Table.setIsView(Boolean.valueOf(attsMap.get("isView"))
						.booleanValue());
				...
				try {
					if (m_Table.save(m_trxName) == true) {
						...
						record_log(1, m_Table.getName(), "Table", m_Table
								.get_ID(), AD_Backup_ID, Object_Status,
								"AD_Table", get_IDWithColumn("AD_Table",
										"TableName", "AD_Table"));
					} else {
						System.out.println("m_Table.save failed");
						record_log(0, m_Table.getName(), "Table", m_Table
								.get_ID(), AD_Backup_ID, Object_Status,
								"AD_Table", get_IDWithColumn("AD_Table",
										"TableName", "AD_Table"));
					}
				} catch (SAXException e) {
					System.out.println("Exception in importTable: "
							+ e.getMessage());
		...
		public void importField(Map<String, String> attsMap) {
			...
			if (entitytype.compareTo("U") == 0
				 ...
				 * //Adjust for difference between Oracle and PostgreSql
				 * DataTypes if (m_DatabaseType.equals("Oracle")){ if
				 * (colname.equals("TIMESTAMP")) colname = "Date"; else if
				 * (colname.equals("TIMESTAMP+Time")) colname = "Date+Time";
				 * else if (colname.equals("NUMERIC")) colname = "Number";} else
				 * if (m_DatabaseType.equals("Sybase")){ if
				 * (colname.equals("TIMESTAMP")) colname = "Date"; else if
				 * (colname.equals("TIMESTAMP+Time")) colname = "Date+Time";
				 * else if (colname.equals("NUMERIC")) colname = "Number";} else
				 * if (m_DatabaseType.equals("PostgreSQL")){ log.info("Before
				 * Column Name ->"+colname); if (colname.equals("Date")) colname =
				 * "TIMESTAMP"; else if (colname.equals("Date+Time")) colname =
				 * "TIMESTAMP+Time"; else if (colname.equals("Number")) colname =
				 * "NUMERIC"; System.out.println("After Column Name
				 * ->"+colname); }
				 */
				int tableid = get_IDWithColumn("AD_Table", "TableName", attsMap
						.get("ADTableNameID"));
				int windowid = get_ID("AD_Window", attsMap
						.get("ADWindowNameID"));
				// log.info("Column Name ->"+colname);
				int columnid = get_IDWithMasterAndColumn("AD_Column",
						"ColumnName", colname, "AD_Table", tableid);
				// log.info("ColumnID->"+columnid);
				sqlB = new StringBuffer(
						"select AD_Tab_ID from AD_Tab where AD_Window_ID = "
								+ windowid + " and Name = '" + tabname + "'"
								+ " and AD_Table_ID = ?");
				...
				if (tabid > 0) {
					...
					m_Field.setIsSameLine(Boolean.valueOf(
							attsMap.get("SameLine")).booleanValue());
					m_Field.setIsCentrallyMaintained(Boolean.valueOf(
							attsMap.get("isCentrallyMaintained"))
							.booleanValue());
					m_Field.setIsDisplayed(Boolean.valueOf(
							attsMap.get("Displayed")).booleanValue());
					// m_Field.setIsEncrypted(Boolean.valueOf(atts.getValue("isEncrypted")).booleanValue());
					m_Field.setIsFieldOnly(Boolean.valueOf(
							attsMap.get("isFieldOnly")).booleanValue());
					m_Field.setIsHeading(Boolean.valueOf(
							attsMap.get("isHeading")).booleanValue());
					m_Field.setIsReadOnly(Boolean.valueOf(
							attsMap.get("isReadOnly")).booleanValue());
					m_Field.setSeqNo(Integer.parseInt(attsMap.get("SeqNo")));
					m_Field.setDisplayLength(Integer.parseInt(attsMap
							.get("DisplayLength")));
					m_Field.setDescription(attsMap.get("Description")
							.replaceAll("'", "''").replaceAll(",", ""));
					m_Field.setHelp(attsMap.get("Help").replaceAll("'", "''")
							.replaceAll(",", ""));
					m_Field
							.setIsActive(attsMap.get("isActive") != null ? Boolean
									.valueOf(attsMap.get("isActive"))
					...
					try {
						if (m_Field.save(m_trxName) == true) {
							...
							record_log(1, m_Field.getName(), "Field", m_Field
									.get_ID(), AD_Backup_ID, Object_Status,
									"AD_Field", get_IDWithColumn("AD_Table",
											"TableName", "AD_Field"));
						} else {
							System.out.println("m_Field.save failed");
							record_log(0, m_Field.getName(), "Field", m_Field
									.get_ID(), AD_Backup_ID, Object_Status,
									"AD_Field", get_IDWithColumn("AD_Table",
											"TableName", "AD_Field"));
						}
					} catch (Exception e) {
						System.out.println("Exception in m_Field.save: "
								+ e.getMessage());

					}
				} else
					System.out.println("Tab do not exist to field Name: "
							+ name);
		...
		public void importPrintFormatItem(Map<String, String> attsMap) {
			...
			String name = attsMap.get("Name");
			int id = get_IDWithMaster("AD_PrintFormatItem", name,
					"AD_PrintFormat", attsMap.get("ADPrintFormatNameID"));

			m_PrintFormatItem = new X_AD_PrintFormatItem(m_ctx, id, m_trxName);
			if (id > 0) {
				AD_Backup_ID = copyRecord("AD_PrintFormatItem",
						m_PrintFormatItem);
				Object_Status = "Update";
			} else {
				Object_Status = "New";
			...
			id = get_IDWithMasterAndColumn("AD_Column", "ColumnName", name,
					"AD_Table", tableid);
			if (id > 0)
			...
			m_PrintFormatItem
					.setPrintFormatType(attsMap.get("PrintFormatType"));
			m_PrintFormatItem
					.setXSpace(Integer.parseInt(attsMap.get("XSpace")));

			m_PrintFormatItem
					.setYSpace(Integer.parseInt(attsMap.get("YSpace")));
			m_PrintFormatItem.setXPosition(Integer.parseInt(attsMap
					.get("Xposition")));
			m_PrintFormatItem.setYPosition(Integer.parseInt(attsMap
					.get("Yposition")));

			m_PrintFormatItem.setMaxWidth(Integer.parseInt(attsMap
					.get("MaxWidth")));
			m_PrintFormatItem.setMaxHeight(Integer.parseInt(attsMap
					.get("MaxHieght")));
			m_PrintFormatItem
					.setSortNo(Integer.parseInt(attsMap.get("SortNo")));

			m_PrintFormatItem.setFieldAlignmentType(attsMap
					.get("FieldAlignmentType"));
			m_PrintFormatItem.setLineAlignmentType(attsMap
					.get("LineAlignmentType"));
			m_PrintFormatItem.setImageURL(attsMap.get("ImageURL"));
			m_PrintFormatItem.setArcDiameter(Integer.parseInt(attsMap
					.get("ArcDiameter")));
			m_PrintFormatItem.setLineWidth(Integer.parseInt(attsMap
					.get("LineWidth")));
			m_PrintFormatItem.setShapeType(attsMap.get("ShapeType"));

			m_PrintFormatItem.setBelowColumn(Integer.parseInt(attsMap
					.get("BelowColumn")));
			m_PrintFormatItem
					.setPrintNameSuffix(attsMap.get("PrintNameSuffix"));
			m_PrintFormatItem.setRunningTotalLines(Integer.parseInt(attsMap
					.get("RunningTotalLines")));

			m_PrintFormatItem
					.setIsActive(attsMap.get("isActive") != null ? Boolean
							.valueOf(attsMap.get("isActive")).booleanValue()
							: true);
			m_PrintFormatItem.setIsPrinted(Boolean.valueOf(
					attsMap.get("isPrinted")).booleanValue());
			m_PrintFormatItem.setIsRelativePosition(Boolean.valueOf(
					attsMap.get("isRelativePosition")).booleanValue());
			m_PrintFormatItem.setIsNextLine(Boolean.valueOf(
					attsMap.get("isNextLine")).booleanValue());

			m_PrintFormatItem.setIsHeightOneLine(Boolean.valueOf(
					attsMap.get("isHeightOneLine")).booleanValue());
			m_PrintFormatItem.setIsOrderBy(Boolean.valueOf(
					attsMap.get("isOrderBy")).booleanValue());
			m_PrintFormatItem.setIsGroupBy(Boolean.valueOf(
					attsMap.get("isGroupBy")).booleanValue());

			m_PrintFormatItem.setIsPageBreak(Boolean.valueOf(
					attsMap.get("isPageBreak")).booleanValue());
			m_PrintFormatItem.setIsSummarized(Boolean.valueOf(
					attsMap.get("isSummarized")).booleanValue());
			m_PrintFormatItem.setImageIsAttached(Boolean.valueOf(
					attsMap.get("isImageIsAttached")).booleanValue());

			m_PrintFormatItem.setIsAveraged(Boolean.valueOf(
					attsMap.get("isAveraged")).booleanValue());
			m_PrintFormatItem.setIsCounted(Boolean.valueOf(
					attsMap.get("isCounted")).booleanValue());
			m_PrintFormatItem.setIsSetNLPosition(Boolean.valueOf(
					attsMap.get("isSetNLPosition")).booleanValue());
			m_PrintFormatItem.setIsSuppressNull(Boolean.valueOf(
					attsMap.get("isSuppressNull")).booleanValue());

			m_PrintFormatItem.setIsFixedWidth(Boolean.valueOf(
					attsMap.get("isFixedWidth")).booleanValue());
			m_PrintFormatItem.setIsNextPage(Boolean.valueOf(
					attsMap.get("isNextPage")).booleanValue());
			m_PrintFormatItem.setIsMaxCalc(Boolean.valueOf(
					attsMap.get("isMaxCalc")).booleanValue());
			m_PrintFormatItem.setIsMinCalc(Boolean.valueOf(
					attsMap.get("isMinCalc")).booleanValue());

			m_PrintFormatItem.setIsRunningTotal(Boolean.valueOf(
					attsMap.get("isRunningTotal")).booleanValue());
			m_PrintFormatItem.setIsVarianceCalc(Boolean.valueOf(
					attsMap.get("isVarianceCalc")).booleanValue());
			m_PrintFormatItem.setIsDeviationCalc(Boolean.valueOf(
					attsMap.get("isDeviationCalc")).booleanValue());

			try {

				if (m_PrintFormatItem.save(m_trxName) == true) {
					System.out.println("m_PrintFormatItem.save succeeded");
					record_log(1, m_PrintFormatItem.getName(),
							"PrintFormatItem", m_PrintFormatItem.get_ID(),
							AD_Backup_ID, Object_Status, "AD_PrintFormatItem",
							get_IDWithColumn("AD_Table", "TableName",
									"AD_PrintFormatItem"));
				} else {
					System.out.println("m_PrintFormatItem.save failed");
					record_log(0, m_PrintFormatItem.getName(),
							"PrintFormatItem", m_PrintFormatItem.get_ID(),
							AD_Backup_ID, Object_Status, "AD_PrintFormatItem",
							get_IDWithColumn("AD_Table", "TableName",
									"AD_PrintFormatItem"));
				...
				System.out.println("Exception in m_PrintFormatItem.save: "
						+ e.getMessage());
		...
		public void importPrintFormat(Map<String, String> attsMap) {
			...
			if (id == 0) {
				...
				try {
					...
					System.out.println("Exception in importPrintFormat: "
							+ e.getMessage());
			...
			m_PrintFormat.setDescription(attsMap.get("Description").replaceAll(
					"'", "''").replaceAll(",", ""));
			m_PrintFormat.setName(attsMap.get("Name"));
			m_PrintFormat.setPrinterName(attsMap.get("PrinterName"));
			m_PrintFormat.setFooterMargin(Integer.parseInt(attsMap
					.get("FooterMargin")));

			m_PrintFormat.setHeaderMargin(Integer.parseInt(attsMap
					.get("HeaderMargin")));
			m_PrintFormat.setCreateCopy(attsMap.get("CreateCopy"));
			m_PrintFormat.setIsActive(attsMap.get("isActive") != null ? Boolean
					.valueOf(attsMap.get("isActive")).booleanValue() : true);

			m_PrintFormat.setIsTableBased(Boolean.valueOf(
					attsMap.get("isTableBased")).booleanValue());
			m_PrintFormat.setIsForm(Boolean.valueOf(attsMap.get("isForm"))
					.booleanValue());
			m_PrintFormat.setIsStandardHeaderFooter(Boolean.valueOf(
					attsMap.get("isStandardHeader")).booleanValue());

			m_PrintFormat.setIsDefault(Boolean
					.valueOf(attsMap.get("isDefault")).booleanValue());
			try {
				if (m_PrintFormat.save(m_trxName) == true) {
					System.out.println("m_PrintFormat.save succeeded");
					record_log(1, m_PrintFormat.getName(), "PrintFormat",
							m_PrintFormat.get_ID(), AD_Backup_ID,
							Object_Status, "AD_PrintFormat", get_IDWithColumn(
									"AD_Table", "TableName", "AD_PrintFormat"));
				} else {
					System.out.println("m_PrintFormat.save failed");
					record_log(0, m_PrintFormat.getName(), "PrintFormat",
							m_PrintFormat.get_ID(), AD_Backup_ID,
							Object_Status, "AD_PrintFormat", get_IDWithColumn(
									"AD_Table", "TableName", "AD_PrintFormat"));
				}
			} catch (Exception e) {
				System.out.println("Exception with importPrintFormat: "
						+ e.getMessage());
		...
		public void importWorkflowNodeNext(Map<String, String> attsMap) {
			...
			if (entitytype.equals("U") || entitytype.equals("D")
				...
				int workflowId = get_IDWithColumn("AD_Workflow", "name",
						workflowName);

				String workflowNodeName = attsMap.get("ADWorkflowNodeNameID");
				String workflowNodeNextName = attsMap
						.get("ADWorkflowNodeNextNameID");

				System.out.println("workflowNodeNextName: "
						+ workflowNodeNextName);

				// analyzeWorkflowImport( workflowId, workflowNodeName,
				// m_trxName);

				sqlB = new StringBuffer(
						"SELECT ad_wf_node_id FROM AD_WF_Node WHERE AD_Workflow_ID=? and Name =?");

				int wfNodeId = DB.getSQLValue(m_trxName, sqlB.toString(),
						workflowId, workflowNodeName);
				System.out.println("wfNodeId found: " + wfNodeId);
				if (wfNodeId < 0) {
					// wfNodeId = createPlaceHolderWorkflowNode( atts,
					// m_trxName);
				}

				int wfNodeNextId = DB.getSQLValue(m_trxName, sqlB.toString(),
						workflowId, workflowNodeNextName);

				sqlB = new StringBuffer(
						"SELECT  ad_wf_nodenext_id FROM AD_WF_NodeNext  WHERE ad_wf_node_id =? and ad_wf_next_id =?");

				// int id = get_IDWithColumn("AD_WF_Node", "name",
				// workflowNodeName);
				int id = DB.getSQLValue(m_trxName, sqlB.toString(), wfNodeId,
						wfNodeNextId);
				...
				System.out
						.println("about to execute m_WFNodeNext.setAD_WF_Node_ID with wfNodeId: "
								+ wfNodeId);
				...
				m_WFNodeNext
						.setIsActive(attsMap.get("isActive") != null ? Boolean
								...
								.booleanValue() : true);
				m_WFNodeNext
						.setIsStdUserWorkflow(attsMap.get("IsStdUserWorkflow") != null ? Boolean
								.valueOf(attsMap.get("IsStdUserWorkflow"))
								.booleanValue()
								: true);
				// attsOut.clear();
				...
				try {
					if (m_WFNodeNext.save(m_trxName) == true) {
						...
						record_log(1, String.valueOf(m_WFNodeNext.get_ID()),
								"WFNodeNext", m_WFNodeNext.get_ID(),
								AD_Backup_ID, Object_Status, "AD_WF_NodeNext",
								get_IDWithColumn("AD_WF_NodeNext",
										"ad_wf_nodenext_id", "AD_WF_NodeNext"));
					} else {
						System.out.println("m_WFNodeNext save failure");
						record_log(0, String.valueOf(m_WFNodeNext.get_ID()),
								"WFNode", m_WFNodeNext.get_ID(), AD_Backup_ID,
								Object_Status, "AD_WF_NodeNext",
								get_IDWithColumn("AD_WF_NodeNext",
										"ad_wf_nodenext_id", "AD_WF_NodeNext"));
					}
				} catch (SAXException e) {
					System.out.println("Exception in importWorkflowNodeNext: "
							+ e.getMessage());
		...
		public void importWorkflowNode(Map<String, String> attsMap) {
			...
			if (entitytype.equals("U") || entitytype.equals("D")
				...
				int id = DB.getSQLValue(m_trxName, sqlB.toString(), workflowId,
						workflowNodeName);
				...
				if (attsMap.get("ADProcessNameID") != null) {
					...
					id = get_IDWithColumn("AD_Process", "Name", Name);
					if (id <= 0
							&& attsMap.get("Action").equals(
									X_AD_WF_Node.ACTION_AppsProcess)
							|| attsMap.get("Action").equals(
									X_AD_WF_Node.ACTION_AppsReport)) {
				...
				if (attsMap.get("ADWindowNameID") != null) {
					...
					if (id <= 0
							&& attsMap.get("Action").equals(
									X_AD_WF_Node.ACTION_UserWindow)) {
				 ...
				 * (attsMap.get("ADTaskNameID")!=null){ String name =
				 * atts.getValue("ADTaskNameID"); sqlB = new StringBuffer
				 * ("SELECT AD_Task_ID FROM AD_Task WHERE Name= ?"); taskid =
				...
				m_WFNode.setDescription(attsMap.get("Description").replaceAll(
						"'", "''").replaceAll(",", ""));
				m_WFNode.setJoinElement(attsMap.get("JoinElement"));
				m_WFNode.setSplitElement(attsMap.get("SplitElement"));
				m_WFNode
						.setXPosition(Integer.valueOf(attsMap.get("XPosition")));
				m_WFNode
						.setYPosition(Integer.valueOf(attsMap.get("YPosition")));
				m_WFNode.setWaitingTime(Integer.valueOf(attsMap
						.get("WaitingTime")));
				m_WFNode.setWaitTime(Integer.valueOf(attsMap.get("WaitTime")));
				m_WFNode.setWorkingTime(Integer.valueOf(attsMap
						.get("WorkingTime")));
				...
				m_WFNode.setIsCentrallyMaintained(Boolean.valueOf(
						attsMap.get("IsCentrallyMaintained")).booleanValue());
				m_WFNode.setDynPriorityChange(new BigDecimal(attsMap
						.get("DynPriorityChange")));
				// m_WFNode.setAccessLevel (atts.getValue("AccessLevel"));
				// FIXME: Failing for some reason on a ""
				// m_WFNode.setDynPriorityUnit
				// (atts.getValue("DynPriorityUnit"));
				m_WFNode
						.setIsActive(attsMap.get("isActive") != null ? Boolean
								.valueOf(attsMap.get("isActive"))
								.booleanValue() : true);
				...
				try {
					if (m_WFNode.save(m_trxName) == true) {
						...
						record_log(1, m_WFNode.getName(), "WFNode", m_WFNode
								.get_ID(), AD_Backup_ID, Object_Status,
								"AD_WF_Node", get_IDWithColumn("AD_WF_Node",
										"Name", "AD_WF_Node"));
					} else {
						System.out.println("m_WFNode save failure");
						record_log(0, m_WFNode.getName(), "WFNode", m_WFNode
								.get_ID(), AD_Backup_ID, Object_Status,
								"AD_WF_Node", get_IDWithColumn("AD_WF_Node",
										"Name", "AD_WF_Node"));
					}
				} catch (SAXException e) {
					System.out.println("Exception in importWorkflowNode: "
							+ e.getMessage());
		...
		public void importWorkflow(Map<String, String> attsMap) {
			...
			if (entitytype.equals("U") || entitytype.equals("D")) {
				...
				m_Workflow.setDescription(attsMap.get("Description")
						.replaceAll("'", "''").replaceAll(",", ""));
				m_Workflow.setHelp(attsMap.get("Help").replaceAll("'", "''")
						.replaceAll(",", ""));
				...
				m_Workflow
						.setPriority(Integer.valueOf(attsMap.get("Priority")));
				m_Workflow.setLimit(Integer.valueOf(attsMap.get("Limit")));
				m_Workflow
						.setDuration(Integer.valueOf(attsMap.get("Duration")));
				m_Workflow.setCost(Integer.valueOf(attsMap.get("Cost")));
				m_Workflow.setWorkingTime(Integer.valueOf(attsMap
						.get("WorkingTime")));
				m_Workflow.setWaitingTime(Integer.valueOf(attsMap
						.get("WaitingTime")));
				...
				m_Workflow.setIsValid(attsMap.get("isValid") != null ? Boolean
						.valueOf(attsMap.get("isValid")).booleanValue() : true);
				...
				try {
					if (m_Workflow.save(m_trxName) == true) {
						...
						record_log(1, m_Workflow.getName(), "Workflow",
								m_Workflow.get_ID(), AD_Backup_ID,
								Object_Status, "AD_Workflow", get_IDWithColumn(
										"AD_Workflow", "Name", "AD_Workflow"));
					} else {
						System.out.println("m_Workflow save failure");
						record_log(0, m_Workflow.getName(), "Workflow",
								m_Workflow.get_ID(), AD_Backup_ID,
								Object_Status, "AD_Workflow", get_IDWithColumn(
										"AD_Workflow", "Name", "AD_Workflow"));
					}
				} catch (SAXException e) {
					System.out.println("Exception in importWorkflow: "
							+ e.getMessage());
		...
		public void importProcess(Map<String, String> attsMap) {
			...
			if (entitytype.compareTo("U") == 0
				...
				if (id > 0) {
					...
					m_Process = new MProcess(m_ctx, id, m_trxName);
					id = MSequence.getNextID(Env.getAD_Client_ID(m_ctx),
							"AD_Process", m_trxName);
				...
				m_Process.setDescription(attsMap.get("Description").replaceAll(
						"'", "''").replaceAll(",", ""));
				m_Process.setEntityType(attsMap.get("EntityType"));
				m_Process.setHelp(attsMap.get("Help").replaceAll("'", "''")
						.replaceAll(",", ""));
				m_Process.setIsBetaFunctionality(Boolean.valueOf(
						attsMap.get("isBetaFunctionality")).booleanValue());
				m_Process.setIsDirectPrint(Boolean.valueOf(
						attsMap.get("isDirectPrint")).booleanValue());
				m_Process.setIsReport(Boolean.valueOf(attsMap.get("isReport"))
						.booleanValue());
				...
				try {
					if (m_Process.save(m_trxName) == true) {
						...
						record_log(1, m_Process.getName(), "Process", m_Process
								.get_ID(), AD_Backup_ID, Object_Status,
								"AD_Process", get_IDWithColumn("AD_Table",
										"TableName", "AD_Process"));
					} else {
						System.out.println("m_Process.save failed");
						record_log(0, m_Process.getName(), "Process", m_Process
								.get_ID(), AD_Backup_ID, Object_Status,
								"AD_Process", get_IDWithColumn("AD_Table",
										"TableName", "AD_Process"));
					}
				} catch (SAXException e) {
					System.out.println("Exception in importProcess: "
							+ e.getMessage());
		...
		public void importMenu(Map<String, String> attsMap) {
			...
			String entitytype = attsMap.get("EntityType");
			// if (entitytype.compareTo("U") == 0 || entitytype.compareTo("D")
			// == 0) {
			...
			m_Menu.setName(name);
			int id = get_IDWithColumn("AD_Window", "Name", attsMap
					.get("ADWindowNameID"));
			m_Menu.setAD_Window_ID(id);
			id = get_IDWithColumn("AD_Process", "Name", attsMap
					.get("ADProcessNameID"));
			m_Menu.setAD_Process_ID(id);
			id = get_IDWithColumn("AD_Form", "Name", attsMap
					.get("ADFormNameID"));
			m_Menu.setAD_Form_ID(id);
			id = get_IDWithColumn("AD_Task", "Name", attsMap
					.get("ADTaskNameID"));
			m_Menu.setAD_Task_ID(id);

			id = get_IDWithColumn("AD_Workbench", "Name", attsMap
					.get("ADWorkbenchNameID"));
			m_Menu.setAD_Workbench_ID(id);
			id = get_IDWithColumn("AD_Workflow", "Name", attsMap
					.get("ADWorkflowNameID"));
			m_Menu.setAD_Workflow_ID(id);

			String m_Action = (attsMap.get("Action") != null ? attsMap
					.get("Action") : " ");
			if (m_Action.compareTo(" ") > -1)
				m_Menu.setAction(m_Action);

			m_Menu.setDescription(attsMap.get("Description").replaceAll("'",
					"''").replaceAll(",", ""));
			m_Menu.setEntityType(attsMap.get("EntityType"));
			m_Menu.setIsReadOnly(Boolean.valueOf(attsMap.get("isReadOnly"))
					.booleanValue());
			m_Menu.setIsSOTrx(Boolean.valueOf(attsMap.get("isSOTrx"))
					.booleanValue());
			m_Menu.setIsSummary(Boolean.valueOf(attsMap.get("isSummary"))
					.booleanValue());
			m_Menu.setIsActive(Boolean.valueOf(attsMap.get("isActive"))
					.booleanValue());
			...
			if (m_Menu.save(m_trxName) == true) {
				try {
					...
					idDetail = record_log(
							...
							Object_Status,
							"AD_Menu",
							get_IDWithColumn("AD_Table", "TableName", "AD_Menu"));
				} catch (SAXException e) {
			...
			if (countRecords > 0) {
				StringBuffer sqlC = new StringBuffer(
						"select * from AD_TREENODEMM where AD_Tree_ID = 10 and "
								+ " Node_ID =?");
				try {
					PreparedStatement pstmt1 = DB.prepareStatement(sqlC
							.toString(), m_trxName);
					...
					if (rs1.next()) {
						...
						int tableID = get_IDWithColumn("AD_Table", "TableName",
								"AD_TreeNodeMM");
						...
						for (int q = 1; q <= columns; q++) {
							...
							StringBuffer sqlD = new StringBuffer(
									"Insert INTO AD_Package_Imp_Backup"
											...
											+ AD_Package_Imp_ID + ", "
											+ tableID + ", " + columnID + ", "
											+ referenceID + ", '" + colValue
											+ "')");
							int no = DB.executeUpdate(sqlD.toString(),
									m_trxName);
							if (no == -1)
								System.out
										.println("Insert to import backup failed");
						}
				...
				sqlB = new StringBuffer("UPDATE AD_TREENODEMM "
						+ "SET Parent_ID = " + id + " , SeqNo = "
						+ attsMap.get("ADParentSeqno")
						+ " WHERE AD_Tree_ID = 10" + " AND Node_ID = "
			...
			if (entitytype.compareTo("U") == 0
				...
				if (tableId == 0) {
					...
					try {
						...
						System.out
								.println("Exception in importReferenceTable: "
										+ e.getMessage());

					}
					tableId = get_IDWithColumn("AD_Table", "TableName", attsMap
							.get("ADTableNameID"));
				}
				name = attsMap.get("ADDisplay");
				int DisplayId = get_IDWithMasterAndColumn("AD_Column",
						"ColumnName", name, "AD_Table", tableId);
				if (DisplayId == 0) {
					m_Column = new MColumn(m_ctx, 0, m_trxName);
					m_Column.setAD_Table_ID(tableId);
					// m_Column.setVersion(new BigDecimal("1")); // use
					// constructor value
					...
					try {
						if (m_Column.save(m_trxName) == true) {
							record_log(1, m_Column.getName(), "Column",
									m_Column.get_ID(), 0, "New", "AD_Column",
									get_IDWithColumn("AD_Table", "TableName",
											"AD_Column"));
						} else {
							record_log(0, m_Column.getName(), "Column",
									m_Column.get_ID(), 0, "New", "AD_Column",
									get_IDWithColumn("AD_Table", "TableName",
											"AD_Column"));
						}
				...
				int keyId = get_IDWithMasterAndColumn("AD_Column",
						"ColumnName", name, "AD_Table", tableId);
				if (keyId == 0) {
					m_Column = new MColumn(m_ctx, 0, m_trxName);
					m_Column.setAD_Table_ID(tableId);
				...
				name = attsMap.get("ADDisplay");
				DisplayId = get_IDWithMasterAndColumn("AD_Column",
						"ColumnName", name, "AD_Table", tableId);
				name = attsMap.get("Key");
				keyId = get_IDWithMasterAndColumn("AD_Column", "ColumnName",
						name, "AD_Table", tableId);
				String entityType = attsMap.get("EntityType");
				String isValueDisplayed = attsMap.get("IsValueDisplayed");
				String OrderByClause = attsMap.get("OrderByClause").replaceAll(
						"'", "''").replaceAll(",", "");
				String WhereClause = attsMap.get("WhereClause").replaceAll("'",
						"''").replaceAll(",", "");
				if (count > 0) {
					sqlB = new StringBuffer("UPDATE AD_Ref_Table "
							+ "SET AD_Table_ID = " + tableId
							+ ", AD_Display = " + DisplayId + ", AD_Key = "
							+ keyId + ", isValueDisplayed = '"
							+ isValueDisplayed + "', OrderByClause = '"
							+ OrderByClause + "', EntityType ='" + entityType
					...
					try {
						if (no > 0) {
							record_log(1, attsMap.get("ADRefenceNameID"),
									"Reference Table", id, 0, "Update",
									"AD_Ref_Table", get_IDWithColumn(
											"AD_Table", "TableName",
											"AD_Ref_Table"));
						} else {
							record_log(0, attsMap.get("ADRefenceNameID"),
									"Reference Table", id, 0, "Update",
									"AD_Ref_Table", get_IDWithColumn(
											"AD_Table", "TableName",
											"AD_Ref_Table"));
					...
					sqlB = new StringBuffer(
							"Insert INTO AD_Ref_Table"
									...
									+ " WhereClause )" + "VALUES(0, 0, 0, 0, "
									+ id + ", " + tableId + ", " + DisplayId
									+ ", " + keyId + ", '" + entityType
									+ "', '" + isValueDisplayed + "', '"
		...
		public void importProcessPara(Map<String, String> attsMap) {
			...
			if (entitytype.compareTo("U") == 0
				...
				int id = get_IDWithMaster("AD_Process_Para", name,
						"AD_Process", attsMap.get("ADProcessNameID"));
				X_AD_Process_Para m_Process_para = new X_AD_Process_Para(m_ctx,
						id, m_trxName);
				if (id > 0) {
				...
				m_Process_para.setDescription(attsMap.get("Description")
						.replaceAll("'", "''").replaceAll(",", ""));
				m_Process_para.setEntityType(attsMap.get("EntityType"));
				m_Process_para.setHelp(attsMap.get("Help")
						.replaceAll("'", "''").replaceAll(",", ""));
				m_Process_para
						.setIsActive(attsMap.get("isActive") != null ? Boolean
								.valueOf(attsMap.get("isActive"))
				...
				m_Process_para.setFieldLength(Integer.parseInt(attsMap
						.get("FieldLength")));
				m_Process_para.setIsCentrallyMaintained(Boolean.valueOf(
						attsMap.get("isCentrallyMaintained")).booleanValue());
				m_Process_para.setIsMandatory(Boolean.valueOf(
						attsMap.get("isMandatory")).booleanValue());
				m_Process_para.setIsRange(Boolean.valueOf(
						attsMap.get("isRange")).booleanValue());
				try {
					if (m_Process_para.save(m_trxName) == true) {
						System.out.println("m_Process_para.save succeeded");
						record_log(1, m_Process_para.getName(), "Process_para",
								m_Process_para.get_ID(), AD_Backup_ID,
								Object_Status, "AD_Process_para",
								get_IDWithColumn("AD_Table", "TableName",
										"AD_Process_para"));
					} else {
						System.out.println("m_Process_para.save failed");
						record_log(0, m_Process_para.getName(), "Process_para",
								m_Process_para.get_ID(), AD_Backup_ID,
								Object_Status, "AD_Process_para",
								get_IDWithColumn("AD_Table", "TableName",
										"AD_Process_para"));
					...
					System.out.println("Exception in importProcessPara: "
							+ e.getMessage());
		...
		public void importPreference(Map<String, String> attsMap) {
			...
			sqlB = new StringBuffer(
					"select AD_Preference_ID from AD_Preference where "
			...
			try {
				if (m_Preference.save(m_trxName) == true) {
					...
					record_log(1, m_Preference.getAttribute(), "Preference",
							m_Preference.get_ID(), AD_Backup_ID, Object_Status,
							"AD_Preference", get_IDWithColumn("AD_Table",
									"TableName", "AD_Preference"));
				} else {
					System.out.println("m_Preference.save failed");
					record_log(0, m_Preference.getAttribute(), "Preference",
							m_Preference.get_ID(), AD_Backup_ID, Object_Status,
							"AD_Preference", get_IDWithColumn("AD_Table",
									"TableName", "AD_Preference"));
				}
			} catch (SAXException e) {
				System.out.println("Exception in importPreference: "
						+ e.getMessage());
		...
		public void importTab(Map<String, String> attsMap) {
			...
			if (entitytype.compareTo("U") == 0
				...
				if (attsMap.get("ADColumnNameID") != null) {
					name = attsMap.get("ADColumnNameID");
					id = get_IDWithMasterAndColumn("AD_Column", "Name", attsMap
							.get("ADColumnNameID"), "AD_Table",
							get_IDWithColumn("AD_Table", "TableName", attsMap
									.get("ADTableNameID")));
				...
				m_Tab.setDescription(attsMap.get("Description").replaceAll("'",
						"''").replaceAll(",", ""));
				m_Tab.setEntityType(attsMap.get("EntityType"));
				m_Tab.setHasTree(Boolean.valueOf(attsMap.get("isHasTree"))
						.booleanValue());
				m_Tab.setHelp(attsMap.get("Help").replaceAll("'", "''")
						.replaceAll(",", ""));
				m_Tab
						.setIsActive(attsMap.get("isActive") != null ? Boolean
								.valueOf(attsMap.get("isActive"))
				...
				m_Tab.setIsInfoTab(Boolean.valueOf(attsMap.get("isInfoTab"))
						.booleanValue());
				m_Tab.setIsReadOnly(Boolean.valueOf(attsMap.get("isReadOnly"))
						.booleanValue());
				m_Tab.setIsSingleRow(Boolean
						.valueOf(attsMap.get("isSingleRow")).booleanValue());
				m_Tab.setIsSortTab(Boolean.valueOf(attsMap.get("isSortTab"))
						.booleanValue());
				m_Tab.setIsTranslationTab(Boolean.valueOf(
						attsMap.get("IsTranslationTab")).booleanValue());
				...
				if (attsMap.get("isInsertRecord") != null) {
					m_Tab.setIsInsertRecord(Boolean.valueOf(
							attsMap.get("isInsertRecord")).booleanValue());
				}
				if (attsMap.get("isAdvancedTab") != null) {
					m_Tab.setIsAdvancedTab(Boolean.valueOf(
							attsMap.get("isAdvancedTab")).booleanValue());
				}

				try {
					if (m_Tab.save(m_trxName) == true) {
						System.out.println("m_Tab.save succeeded");
						record_log(1, m_Tab.getName(), "Tab", m_Tab.get_ID(),
								AD_Backup_ID, Object_Status, "AD_Tab",
								get_IDWithColumn("AD_Table", "TableName",
										"AD_Tab"));
					} else {
						System.out.println("m_Tab.save failed");
						record_log(0, m_Tab.getName(), "Tab", m_Tab.get_ID(),
								AD_Backup_ID, Object_Status, "AD_Tab",
								get_IDWithColumn("AD_Table", "TableName",
										"AD_Tab"));
					}
				} catch (SAXException e) {
					System.out.println("Exception in importTab: "
							+ e.getMessage());
		...
		public void importWindow(Map<String, String> attsMap) {
			...
			if (entitytype.compareTo("U") == 0
				...
				m_Window.setDescription(attsMap.get("Description").replaceAll(
						"'", "''").replaceAll(",", ""));
				m_Window.setEntityType(attsMap.get("EntityType"));
				m_Window.setHelp(attsMap.get("Help").replaceAll("'", "''")
						.replaceAll(",", ""));
				m_Window
						.setIsActive(attsMap.get("isActive") != null ? Boolean
								.valueOf(attsMap.get("isActive"))
				...
				m_Window.setIsBetaFunctionality(Boolean.valueOf(
						attsMap.get("isBetaFunctionality")).booleanValue());
				m_Window.setIsDefault(Boolean.valueOf(attsMap.get("isDefault"))
						.booleanValue());
				m_Window.setIsSOTrx(Boolean.valueOf(attsMap.get("isSOTrx"))
						.booleanValue());
				...
				try {
					if (m_Window.save(m_trxName) == true) {
						...
						record_log(1, m_Window.getName(), "Window", m_Window
								.get_ID(), AD_Backup_ID, Object_Status,
								"AD_Window", get_IDWithColumn("AD_Table",
										"TableName", "AD_Window"));
					} else {
						System.out.println("m_Window.save failed");
						record_log(0, m_Window.getName(), "Window", m_Window
								.get_ID(), AD_Backup_ID, Object_Status,
								"AD_Window", get_IDWithColumn("AD_Table",
										"TableName", "AD_Window"));
					}
				} catch (SAXException e) {
					System.out.println("Exception in importWindow: "
							+ e.getMessage());
		...
		public void importMessage(Map<String, String> attsMap) {
			...
			if (entitytype.equals("U") || entitytype.equals("D")) {
				...
				m_Message.setMsgText(attsMap.get("MsgText").replaceAll("'",
						"''").replaceAll(",", ""));
				m_Message.setMsgTip(attsMap.get("MsgTip").replaceAll("'", "''")
						.replaceAll(",", ""));
				...
				try {
					if (m_Message.save(m_trxName) == true) {
						...
						record_log(1, m_Message.getValue(), "Message",
								m_Message.get_ID(), AD_Backup_ID,
								Object_Status, "AD_Message", get_IDWithColumn(
										"AD_Message", "value", "AD_Message"));
					} else {
						System.out.println("m_Message.save failed");
						record_log(0, m_Message.getValue(), "Message",
								m_Message.get_ID(), AD_Backup_ID,
								Object_Status, "AD_Message", get_IDWithColumn(
										"AD_Message", "value", "AD_Message"));
					}
				} catch (SAXException e) {
					System.out.println("Exception in importMessage: "
							+ e.getMessage());
		...
		public void importDynValRule(Map<String, String> attsMap) {
			...
			if (entitytype.equals("U") || entitytype.equals("D")) {
				...
				m_ValRule.setDescription(attsMap.get("Description").replaceAll(
						"'", "''").replaceAll(",", ""));
				...
				try {
					if (m_ValRule.save(m_trxName) == true) {
						...
						record_log(1, m_ValRule.getName(), "Task", m_ValRule
								.get_ID(), AD_Backup_ID, Object_Status,
								"AD_Val_Rule", get_IDWithColumn("AD_Val_Rule",
										"Name", "AD_Val_Rule"));
					} else {
						System.out.println("m_ValRule.save failed");
						record_log(0, m_ValRule.getName(), "Task", m_ValRule
								.get_ID(), AD_Backup_ID, Object_Status,
								"AD_Val_Rule", get_IDWithColumn("AD_Val_Rule",
										"Name", "AD_Val_Rule"));
					}
				} catch (SAXException e) {
					System.out.println("Exception in importDynValRule: "
							+ e.getMessage());
		...
		public void importReferenceList(Map<String, String> attsMap) {
			...
			if (entitytype.compareTo("U") == 0
					|| entitytype.compareTo("D") == 0) {
				String name = attsMap.get("Name");
				int Referenceid = get_IDWithColumn("AD_Reference", "Name",
						attsMap.get("ADRefenceNameID"));
				int id = get_IDWithMaster("AD_Ref_List", name, "AD_Reference",
						Referenceid);
				...
				m_Ref_List.setDescription(attsMap.get("Description")
						.replaceAll("'", "''").replaceAll(",", ""));
				...
				try {
					if (m_Ref_List.save(m_trxName) == true) {
						...
						record_log(1, m_Ref_List.getName(), "Reference List",
								m_Ref_List.get_ID(), AD_Backup_ID,
								Object_Status, "AD_Ref_List", get_IDWithColumn(
										"AD_Table", "TableName", "AD_Ref_List"));
					} else {
						System.out.println("m_Ref_List.save failed");
						record_log(0, m_Ref_List.getName(), "Reference List",
								m_Ref_List.get_ID(), AD_Backup_ID,
								Object_Status, "AD_Ref_List", get_IDWithColumn(
										"AD_Table", "TableName", "AD_Ref_List"));
					}
				} catch (SAXException e) {
					System.out.println("Exception in importReferenceList: "
							+ e.getMessage());
		...
		public void importReference(Map<String, String> attsMap) {
			...
			if (entitytype.compareTo("U") == 0
				...
				m_Reference.setDescription(attsMap.get("Description")
						.replaceAll("'", "''").replaceAll(",", ""));
				m_Reference.setEntityType(attsMap.get("EntityType"));
				m_Reference.setHelp(attsMap.get("Help").replaceAll("'", "''")
						.replaceAll(",", ""));
				m_Reference
						.setIsActive(attsMap.get("isActive") != null ? Boolean
								.valueOf(attsMap.get("isActive"))
				...
				try {
					if (m_Reference.save(m_trxName) == true) {
						...
						record_log(1, m_Reference.getName(), "Reference",
								m_Reference.get_ID(), AD_Backup_ID,
								Object_Status, "AD_Reference",
								get_IDWithColumn("AD_Table", "TableName",
										"AD_Reference"));
					} else {
						System.out.println("m_Reference.save failed");
						record_log(0, m_Reference.getName(), "Reference",
								m_Reference.get_ID(), AD_Backup_ID,
								Object_Status, "AD_Reference",
								get_IDWithColumn("AD_Table", "TableName",
										"AD_Reference"));
		...
		public void handleMenusImport() {
			...
			if (elementAttsMap.containsKey("menu")) {
				...
				List<Map<String, String>> m_menusAttsMapList = elementAttsMap
						.get("menu");
				System.out.println("processing " + m_menusAttsMapList.size()
						+ " menus");
				while (m_menusAttsMapList.size() > 0) {
					Map<String, String> attsMap = m_menusAttsMapList.get(0);
					System.out.println("menu name: "
							+ attsMap.get("ADMenuNameID"));
					importMenu(attsMap);
					attsMap.clear();
		...
		public void handleWorkflowNodeNextsImport() {
			...
			if (elementAttsMap.containsKey("workflowNodeNext")) {
				...
				List<Map<String, String>> m_workflowNodeNextsAttsMapList = elementAttsMap
						.get("workflowNodeNext");
				System.out.println("processing "
						+ m_workflowNodeNextsAttsMapList.size()
						+ " workflowNodeNexts");
				int i = 0;
				while (m_workflowNodeNextsAttsMapList.size() > 0) {
					Map<String, String> attsMap = m_workflowNodeNextsAttsMapList
							.get(0);
					System.out.println("worklfowNodeNext num: " + i++
							+ " name: "
							+ attsMap.get("ADWorkflowNodeNextNameID"));
		...
		public void handleReferenceTablesImport() {
			...
			if (elementAttsMap.containsKey("referencetable")) {
				...
				List<Map<String, String>> m_referenceTablesAttsMapList = elementAttsMap
						.get("referencetable");
				System.out.println("processing "
						+ m_referenceTablesAttsMapList.size()
						+ " referenceTables");
				while (m_referenceTablesAttsMapList.size() > 0) {
					Map<String, String> attsMap = m_referenceTablesAttsMapList
							.get(0);
					System.out.println("reference table name: "
							+ attsMap.get("ADRefenceNameID"));
		...
		public void handleProcessParasImport() {
			...
			if (elementAttsMap.containsKey("processpara")) {
				...
				List<Map<String, String>> m_processParasAttsMapList = elementAttsMap
						.get("processpara");
				System.out.println("processing "
						+ m_processParasAttsMapList.size() + " processparas");
				while (m_processParasAttsMapList.size() > 0) {
					Map<String, String> attsMap = m_processParasAttsMapList
							.get(0);
					System.out.println("processPara name: "
							+ attsMap.get("Name"));
		...
		public void handlePreferencesImport() {
			...
			if (elementAttsMap.containsKey("preference")) {
				...
				List<Map<String, String>> m_preferencesAttsMapList = elementAttsMap
						.get("preference");
				System.out.println("processing "
						+ m_preferencesAttsMapList.size() + " preferences");
				while (m_preferencesAttsMapList.size() > 0) {
					Map<String, String> attsMap = m_preferencesAttsMapList
							.get(0);
		...
		public void handleTabsImport() {
			...
			if (elementAttsMap.containsKey("tab")) {
				...
				List<Map<String, String>> m_tabsAttsMapList = elementAttsMap
						.get("tab");
				System.out.println("processing " + m_tabsAttsMapList.size()
						+ " tabs");
		...
		public void processImportStructure() {
			...
			for (Map.Entry<String, List<Map<String, String>>> e : elementAttsMap
					.entrySet()) {
		...
		public int get_ID(String tableName, String name) {
			int id = 0;
			sqlB = new StringBuffer("select " + tableName + "_ID from "
					+ tableName + " where name=?");

			if (!tableName.startsWith("AD_"))
				sqlB = sqlB.append(" and AD_Client_ID=?");
			try {
				PreparedStatement pstmt = DB.prepareStatement(sqlB.toString(),
						m_trxName);
		 ...
		 * @param name
		 */
		public int get_IDWithColumn(String tableName, String columnName,
				Object value) {
			int id = 0;
			sqlB = new StringBuffer("select " + tableName + "_ID from "
					+ tableName + " where UPPER(" + columnName + ")=?");
			// StringBuffer sqlC = new StringBuffer ("select "+tableName+"_ID
			// from "+tableName+" where "+columnName+"="+value.toString());
		 ...
		 * @param nameMaster
		 */
		public int get_IDWithMaster(String tableName, String name,
				String tableNameMaster, String nameMaster) {
			int id = 0;
			sqlB = new StringBuffer("select " + tableName + "_ID from "
					+ tableName + " where UPPER(name)=? and " + tableNameMaster
					+ "_ID = (select " + tableNameMaster + "_ID from "
					+ tableNameMaster + " where UPPER(name)=?)");
		...
		public int get_IDWithMasterAndColumn(String tableName,
				String columnName, String name, String tableNameMaster,
				int masterID) {
			int id = 0;
			sqlB = new StringBuffer("select " + tableName + "_ID from "
					+ tableName + " where UPPER(" + columnName + ")=? and "
					+ tableNameMaster + "_ID =?");
			// StringBuffer sqlC = new StringBuffer ("select "+tableName+"_ID
			// from "+tableName+" where "+columnName+"="+name+" and "
		 ...
		 */
		public int get_IDWithMaster(String tableName, String name,
				String tableNameMaster, int masterID) {
			int id = 0;
			sqlB = new StringBuffer("select " + tableName + "_ID from "
					+ tableName + " where name=? and " + tableNameMaster
					+ "_ID=?");
		 ...
		 * 
		 */
		public int record_log(int success, String objectName,
				String objectType, int objectID, int objectIDBackup,
				String objectStatus, String tableName, int AD_Table_ID)
				throws SAXException {
			...
			if (success == 1) {
				...
				sqlB = new StringBuffer(
						"Insert INTO AD_Package_Imp_Detail"
								...
								+ Env.getAD_Org_ID(m_ctx) + ", "
								+ Env.getAD_User_ID(m_ctx) + ", "
								+ Env.getAD_User_ID(m_ctx) + ", " + id + ", "
								+ AD_Package_Imp_ID + ", '" + objectType
								+ "', '" + objectName + "', '" + objectStatus
								+ "', 'Success'" + ", " + objectID + ", "
								+ objectIDBackup + ", '" + tableName + "', "
								+ AD_Table_ID + ")");
				int no = DB.executeUpdate(sqlB.toString(), m_trxName);
				...
				recordLayout = "Type:" + objectType + "  -   Name:" + tableName
						+ "  -  ID:" + objectID + "  -  Action:" + objectStatus
						+ "  -  Failure";
				// hd_documemt.startElement("","","Success",attsOut);
				hd_documemt.characters(recordLayout.toCharArray(), 0,
						recordLayout.length());
		...
		public int copyRecord(String tableName, PO from) {
			...
			for (int i = 0; i < poInfo.getColumnCount(); i++) {
				...
				int columnID = get_IDWithMasterAndColumn("AD_Column",
						"ColumnName", poInfo.getColumnName(i), "AD_Table",
						tableID);
				StringBuffer sqlD = new StringBuffer(
						"SELECT AD_Reference_ID FROM AD_COLUMN WHERE AD_Column_ID = '"
								+ columnID + "'");
				...
				sqlD = new StringBuffer(
						"SELECT MAX(AD_PACKAGE_IMP_DETAIL_ID) FROM AD_PACKAGE_IMP_DETAIL");
				int idDetail = DB.getSQLValue(m_trxName, sqlD.toString()) + 1;

				if (referenceID == 10 || referenceID == 14 || referenceID == 34
						|| referenceID == 17)
	...
	public void testXMLImportStructure() {
		...
		// CHANGE THIS ACCORDING TO THE 2PACK PACKAGE PACKOUT FILE
		String PackOut = "/Users/fernando/Documents/workspace/dashboard/fc_Dashboard/dict/PackOut.xml";
		File in = new File(PackOut);
		if (!in.exists()) {
			...
			try {
				...
				tf_document = (SAXTransformerFactory) SAXTransformerFactory
						.newInstance();
				try {
				...
				msg = "End Parser";
				System.out.println(msg);
				Map<String, List<Map<String, String>>> elementAttsMap = handler
						.getElementAttsMap();

				/*
				 * for (Map.Entry<String, List<Map<String, String>>> e :
				 * elementAttsMap.entrySet()) { //System.out.println(e.getKey() + ": " +
				 * e.getValue()); List<Map<String, String>> m_attsMapList =
				 * e.getValue(); System.out.println("element: " + e.getKey() );
				 * System.out.println("list size: " + m_attsMapList.size()); for
				 * (int j = 0; j < m_attsMapList.size(); j++ ) { Map<String,
				 * String> attsMap = m_attsMapList.get(j);
				 * System.out.println("attsMap length: " + attsMap.size()); for
				 * (Map.Entry<String, String> f : attsMap.entrySet()) {
				 * System.out.println( "attsMap begin ----------");
				 * System.out.println(f.getKey() + ": " + f.getValue());
				 * System.out.println( "atts end ----------"); }
				 *  }
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out
					.println("Exception with m_XMLImportStructureTest.setUp: "
							+ e.getMessage());
		}
	}
