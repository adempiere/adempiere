...
import org.posterita.model.UDIMDunning;
import org.posterita.model.UDIMDunningLevel;
import org.posterita.model.UDIMDunningRun;
...
public class DunningManager
{
    ...
    public static UDIMDunning createDunningAndLevel(Properties ctx,String dunningName,String desc,String dunningLevelName,String printText,String printNote,int daysAfterDue) throws OperationException
    {
        ...
        UDIMDunning udiDunning =  new UDIMDunning(dunning);
        udiDunning.save();
        ...
        UDIMDunningLevel udiDunningLevel = new UDIMDunningLevel(dunningLevel);
        udiDunningLevel.save();
        return udiDunning;
    }
    ...
    private static int DunningRun(Properties ctx, DunningBean bean) throws OperationException
    {
        ...
        UDIMDunningRun UDIDunningRun =new UDIMDunningRun(dunningRun);
        UDIDunningRun.save();
        ...
        if(bean.getBpartnerId()!=null) 
        {
            ProcessInfoParameter param1[]=
            {
                    ...
                    new ProcessInfoParameter("C_Currency_ID",new BigDecimal(POSTerminalManager.getPOSDefaultSellCurrency(ctx).get_ID()),null,null,null),   
                    new ProcessInfoParameter("C_Currency_ID",new BigDecimal(POSTerminalManager.getPOSDefaultSellCurrency(ctx).get_ID()),null,null,null),
            };
    ...
    private static int getOrCreateDunning(Properties ctx) throws OperationException
    {
        ...
        int dunniingId=0;
            ...
            {
                udiDunning= createDunningAndLevel(ctx,dunningName,description,dunningLevelName,printText,printNote,daysAfterDue);
                dunniingId = udiDunning.getMDunning().get_ID();
            }
            else
            {
                dunniingId=dunningIds[0];
            }
            return dunniingId;
    }
    ...
    /*public static MDunning editCreateDunning(Properties ctx,DunningBean bean,String trxName) throws OperationException
     {
     
     MDunning dunning=null;
     if(bean.getDunningId()==null)
     {
     dunning  = new MDunning(ctx,0,trxName); 
     dunning.setIsDefault(true);
     dunning.setSendDunningLetter(true);
     dunning.setName(bean.getDunningName());
     dunning.setDescription(bean.getDescription());
     
     UDIMDunning udiDunning =  new UDIMDunning(dunning);
     udiDunning.save();
     
     
     MDunningLevel dunningLevel = new MDunningLevel(ctx,0,trxName);
     dunningLevel.setC_Dunning_ID(dunning.get_ID());
     dunningLevel.setName(bean.getDunningLevelName());
     dunningLevel.setDaysAfterDue(new BigDecimal(-9999));
     dunningLevel.setPrintName(bean.getDunningPrintText());
     dunningLevel.setNote(bean.getDunningPrintNote());
     ...
     throw new OperationException("no dunning print format for system");
     if(allIds.length > 0)
     throw new OperationException("more than dunning print format for system");
     dunningLevel.setDunning_PrintFormat_ID(allIds[0]);
     ...
     {
     dunning  = new MDunning(ctx,bean.getDunningId().intValue(),trxName); 
     dunning.setName(bean.getDunningName());
     UDIMDunning udiDunning =  new UDIMDunning(dunning);
     udiDunning.save();
     
     MDunningLevel dunningLevel = new MDunningLevel(ctx,bean.getDunningLevelId().intValue(),trxName);
     dunningLevel.setName(bean.getDunningLevelName());
     dunningLevel.setPrintName(bean.getDunningPrintText());
     dunningLevel.setNote(bean.getDunningPrintNote());
     ...
     public static DunningBean getDunning(Properties ctx)
     {
     DunningBean bean=null;
     int dunningIds [] = MDunning.getAllIDs(MDunning.Table_Name,"AD_CLIENT_ID="+Env.getAD_Client_ID(ctx),null);
     ...
     {
     MDunning dunning = new MDunning(ctx,dunningIds [0],null);
     bean = new DunningBean();
     bean.setDunningId(dunning.get_ID());
     bean.setDunningName(dunning.getName());
     bean.setDescription(dunning.getDescription());
     
     int getAllDunningLevelIds [] = MDunningLevel.getAllIDs(MDunningLevel.Table_Name,"AD_CLIENT_ID="+Env.getAD_Client_ID(ctx)+" and C_DUNNING_ID="+dunning.get_ID(),null);
     MDunningLevel dunningLevel = new MDunningLevel(ctx,getAllDunningLevelIds [0],null);
     
     bean.setDunningLevelId(dunningLevel.get_ID());
     bean.setDunningLevelName(dunningLevel.getName());
     bean.setDunningPrintText(dunningLevel.getPrintName());
     bean.setDunningPrintNote(dunningLevel.getNote());
     }
