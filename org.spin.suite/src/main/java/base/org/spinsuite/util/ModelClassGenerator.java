	...
	private String createHeader (int SPS_Table_ID, StringBuffer sb, StringBuffer mandatory, String packageName)
	{
		...
		start.append("/** Generated Model for ").append(tableName).append(NL)
			...
			//	Standard Constructor
			 ...
			 .append("    {").append(NL)
			 .append("      super (ctx, ").append(keyColumn).append(", conn);").append(NL)
			...
			//	Load Constructor
			 ...
			 .append("    public ").append(className).append(" (Context ctx, Cursor rs, DB conn)").append(NL)
			 .append("    {").append(NL)
			 .append("      super (ctx, rs, conn);").append(NL)
			 .append("    }").append(NL)
		...
		int AD_Table_ID)
	{
		...
		if (isMandatory)
		{
			...
			else if (clazz.equals(Boolean.class))
			{
				...
				mandatory.append("new Date( System.currentTimeMillis() )");
			else
		...
		if (clazz.equals(Integer.class)) {
			...
			addImportClass("org.spinsuite.util.Env");
		}
		else if (clazz.equals(Boolean.class)) {
