...
public final class WMRuleEngine {
	 ...
	 */
	public static Collection<MWMRule> getWMRules(Properties ctx, int AD_Client_ID, String trxName)
	{
		String whereClause = "";
		...
		return new Query(ctx, MWMRule.Table_Name, whereClause, trxName).setOnlyActiveRecords(true)
			.setParameters(new Object[]{parameters})
			.<MWMRule>list();
	}
	 ...
	 * @param boundType
	 */
	private static void validateDefinition(MWMInOutBound line, MWMDefinition definition, String boundType)
	{	
		for(MWMStrategy strategy : MWMStrategy.getByBoundType(line.getCtx(), boundType, line.get_TrxName()))
		{
	...
	protected void registerRules(boolean reset)
	{
		...
		WMRuleInterface rule = getWMRuleFactory(WMRuleFIFO.class.getSimpleName());	
	}
	...
	{		
			...
			{
				log.info("Ignore "+rule.getInOutboundClass()+" doesn't have accounting implementation (WMRule* class)");
			}*/
			//
			try
			{
			...
			{
				s_RuleNoImplement.add(className); // Add to negative cache (won't try to load again)
			}
			catch (Throwable e)
			{
	...
	protected String getClassName(MWMRule rule)
	{
		...
		else if(MWMRule.INOUTBOUNDRULE_CustumInterface.equals(rule.getInOutBoundRule()))
		{
	 ...
	 * @param MWMInOutBoundLine line
	 * @param WM_Area_Type_ID WM Area Type
	 * @param WM_Section_Type_ID Section Type
	 * @return Collection with Locator to Document Bound
	 */
	public Collection<MLocator> getMLocator(MWMInOutBoundLine line , int WM_Area_Type_ID , int WM_Section_Type_ID)
	{		
		MWMStrategy strategy = applyDefinition(line , WM_Area_Type_ID, WM_Section_Type_ID);
		return getMLocators(strategy, line.getMProduct().getM_Product_ID(), WM_Section_Type_ID, WM_Section_Type_ID);
	}

	/**
	 * get Storages
	 * @param MWMInOutBoundLine line
	 * @param WM_Area_Type_ID Area Type
	 * @param WM_Section_Type_ID Section Type
	 ...
	 * @return WMStrategy   
	 */
	private MWMStrategy applyDefinition(MWMInOutBoundLine line ,int WM_Area_Type_ID, int WM_Section_Type_ID) 
	{						
		...
		MWMDefinition definition =  new Query(line.getCtx(), MWMDefinition.Table_Name, whereClause.toString(),line.get_TrxName())
			.setClient_ID()
			.setParameters(
					new Object[]
	...
	/**
	 * get MLocator 
	 * @param MWMStrategy strategy
	 * @param M_Product_ID Product
	 * @param WM_Area_Type_ID Area Type
	 * @param WM_Section_Type_ID Section Type
	 * @return Collection of Locators
	 */
	public static Collection <MLocator> getMLocators(MWMStrategy strategy,int M_Product_ID,int WM_Area_Type_ID , int WM_Section_Type_ID)
	{
