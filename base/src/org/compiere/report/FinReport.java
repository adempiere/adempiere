...
public class FinReport extends FinReportAbstract {
    ...
    private void doCalculations() {
        ...
        for (MReportLine reportLine : reportLines) {
            ...
            if (reportLine.isCalculationTypeAdd()
				...
				int no = DB.executeUpdate(updateReportLine.toString(), trxName);
				if (no != 1)
				...
				/*StringBuffer updateReportLine = new StringBuffer ("UPDATE T_Report r1 SET (");
				...
				if (fixPercentage) {
					try{
						no = DB.executeUpdate(updateFixPorcentage.toString(), trxName);
					}catch (Exception e)	{
					...
					no = DB.executeUpdate(updateReportLine.toString(), trxName);
					if (no != 1) {
        ...
        for (int col = 0; col < reportColumns.length; col++) {
			...
			int no = DB.executeUpdate(updateCalculationColumn.toString(), trxName);
			if (no < 1) {
		...
		if (hasOpposites)
		{
			...
			.append(" AND ABS(LevelNo)<2");			//	0=Line 1=Acct
			int no = DB.executeUpdate(updateReportLineForOppsiteSing.toString(),trxName);
			if (no < 1)
    ...
    private int getFirstValueOrZeroInThere(MReportLine reportLine, final int operation_1) {
        ...
        Trx.run(trxName -> {
            ...
            updateReportLine.append(" FROM T_Report r2 WHERE r2.AD_PInstance_ID = ?")    //.append(getAD_PInstance_ID())
                    .append(" AND r2.PA_ReportLine_ID = ? ")                            //.append(operation_1)
                    .append(" AND r2.Record_ID = ? AND r2.Fact_Acct_ID = ?) "            // AND r2.Record_ID=0 AND r2.Fact_Acct_ID=0)
                            //
                            + "WHERE AD_PInstance_ID = ? ")                                //.append(getAD_PInstance_ID())
                    .append(" AND PA_ReportLine_ID = ? ")                                //.append(reportLine.getPA_ReportLine_ID())
                    .append(" AND ABS(LevelNo) < ? ");            //	ABS(LevelNo)<1 0=Line 1=Acct
            final int levelNo = 1;
    ...
    private int getSecondValue(MReportLine reportLine, final int operation_2) {
        ...
        Trx.run(trxName -> {
            ...
            if (fixPercentage) {
                try {
                    updateFixPercentage.append(" WHERE AD_PInstance_ID = ? ")    //+getAD_PInstance_ID())
                            .append(" AND PA_ReportLine_ID = ? ")                //+ reportLine.getPA_ReportLine_ID())
                            .append(" AND ABS(LevelNo) < ? ");                    //  AND ABS(LevelNo) < 1   0=Line 1=Acct
                    final int levelNo = 1;
    ...
    private int getToggleOppositeSign() {
        ...
        Trx.run(trxName -> {
            ...
            if (hasOpposites) {
                //
                updateReportLineForOppositesSing.append(" WHERE AD_PInstance_ID = ? ")//.append(getAD_PInstance_ID())
                        .append(" AND ABS(LevelNo) < ? ");            //	 AND ABS(LevelNo)<2 0=Line 1=Acct
                final int levelNo = 2;
...
//		log.fine("SeqNo #=" + no);
    ...
    private void insertLineSource(MReportLine reportLine) {
        ...
        for (MReportSource source : reportLine.getSources()) {
			...
			//	
			int no = DB.executeUpdateEx(insertLineSource.toString(), get_TrxName());
			if (CLogMgt.isLevelFinest())
         ...
         Trx.run(trxName -> {
			...
			if(isCombination) {
				combinationId = MAccount.get(getCtx(),
						...
						source.getUser4_ID(),
						source.getUserElement1_ID(),
						source.getUserElement2_ID(), trxName).getC_ValidCombination_ID();
				insertLineSource.append(","+ combinationId +" ");
			}*/
            //	WHERE (sources, posting type)
            //String whereReport = finReport.getWhereClause();
            //StringBuffer whereReportLine = new StringBuffer(getWhereReportLine(reportLine , source, whereReport , isCombination , variable));
			/*StringBuffer whereReportLine = new StringBuffer("");
			if(isCombination) {
				whereReportLine.append(source.getWhereClause(getHierarchyId()));
				whereReportLine.append(getWherePostingType(true));
    ...
    private MPrintFormat getPrintFormat() {
		...
		MPrintFormat printFormat = MPrintFormat.createFromTable(Env.getCtx(), 544 , 0 , get_TrxName());
		...
		Optional<MOrgInfo> maybeOrgInfo = Optional.ofNullable(MOrgInfo.get(Env.getCtx(), organizationId, get_TrxName()));
			...
			Optional.ofNullable(printFormatItem.getName()).ifPresent(printFormatItemName -> {
				...
				*//*if (printFormatItemName.equalsIgnoreCase("page count")) {
				...
				maybeLocation.ifPresent(location -> {
					...
					if (printFormatItemName.contains("@Address4@"column
    ...
    private MPrintFormat synchronizePrintFormatHeader() {
        ...
        Trx.run(trxName -> {
            //final int printFormatId = finReport.getAD_PrintFormat_ID();
            //MPrintFormat printFormat = MPrintFormat.get (getCtx(), printFormatId , true);	//	no cache
            MPrintFormat printFormatHeader = MPrintFormat.get(getCtx(), finReport.getAD_PrintFormatHeader_ID(), true);
            int organizationId = getOrgId() != 0 ? getOrgId() : Env.getAD_Org_ID(Env.getCtx());
