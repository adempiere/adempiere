...
// @SQL=SELECT NVL(MAX(Line),0)+10 AS DefaultValue FROM S_TimeExpenseLine WHERE S_TimeExpense_ID=@S_TimeExpense_ID@
	...
	public I_S_TimeExpense getI_S_TimeExpense() throws Exception 
    {
        Class<?> clazz = MTable.getClass(I_S_TimeExpense.Table_Name);
        I_S_TimeExpense result = null;
        try	{
	        Constructor<?> constructor = null;
	    	constructor = clazz.getDeclaredConstructor(new Class[]{Properties.class, int.class, String.class});
    	    result = (I_S_TimeExpense)constructor.newInstance(new Object[] {getCtx(), new Integer(getS_TimeExpense_ID()), get_TrxName()});
        } catch (Exception e) {
	        log.log(Level.SEVERE, "(id) - Table=" + Table_Name + ",Class=" + clazz, e);
	        log.saveError("Error", "Table=" + Table_Name + ",Class=" + clazz);
           throw e;
	...
	public I_S_TimeType getI_S_TimeType() throws Exception 
    {
        Class<?> clazz = MTable.getClass(I_S_TimeType.Table_Name);
        I_S_TimeType result = null;
        try	{
	        Constructor<?> constructor = null;
	    	constructor = clazz.getDeclaredConstructor(new Class[]{Properties.class, int.class, String.class});
    	    result = (I_S_TimeType)constructor.newInstance(new Object[] {getCtx(), new Integer(getS_TimeType_ID()), get_TrxName()});
        } catch (Exception e) {
	        log.log(Level.SEVERE, "(id) - Table=" + Table_Name + ",Class=" + clazz, e);
	        log.saveError("Error", "Table=" + Table_Name + ",Class=" + clazz);
           throw e;
