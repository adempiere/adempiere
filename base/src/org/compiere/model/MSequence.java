...
public class MSequence extends X_AD_Sequence
{
	 ...
	 */
	public static synchronized int getNextID(int AD_Client_ID,
		String TableName, String trxName)
	{
		...
		if (DB.isRemoteObjects ())
		{
			...
			try
			{
				if (server != null)
				{ // See ServerBean
					// hengsin: don't execute getNextID in tranaction to fix
					// performance and locking issue
		...
		if (CLogMgt.isLevel (LOGLEVEL))
			s_log.log (LOGLEVEL, TableName + " - AdempiereSys=" + adempiereSys
				+ " [" + trxName + "]");
		...
		else
		{
			selectSQL = "SELECT CurrentNext, CurrentNextSys, IncrementNo, AD_Sequence_ID "
				...
				+ "WHERE Name=?"
				+ " AND IsActive='Y' AND IsTableID='Y' AND IsAutoSequence='Y' "
				+ "FOR UPDATE"; // jz derby needs expicitly said it//OF
			// CurrentNext, CurrentNextSys";
			USE_PROCEDURE = true;
		}
		// hengsin: executing getNextID in transaction create huge performance
		// and locking issue
		...
		for (int i = 0; i < 3; i++)
		{
			try
			{
				...
				if (CLogMgt.isLevelFinest ())
					s_log.finest ("AC=" + conn.getAutoCommit () + ", RO="
						+ conn.isReadOnly () + " - Isolation="
						+ conn.getTransactionIsolation () + "("
						+ Connection.TRANSACTION_READ_COMMITTED + ") - RSType="
						+ pstmt.getResultSetType () + "("
						+ ResultSet.TYPE_SCROLL_SENSITIVE + "), RSConcur="
				...
				if (rs.next ())
				{
					...
					// If maintaining official dictionary try to get the ID from
					// http official server
					if (adempiereSys)
					{
						String isUseCentralizedID = MSysConfig.getValue (
							"DICTIONARY_ID_USE_CENTRALIZED_ID", "Y"); // defaults
						// to Y
						if ((!isUseCentralizedID.equals ("N"))
							&& (!isExceptionCentralized (TableName)))
						{
							...
							if (retValue > 0)
							{
								...
								updateSQL = conn
									.prepareStatement ("UPDATE AD_Sequence SET CurrentNextSys = ? + 1 WHERE AD_Sequence_ID = ?");
								try
								{
					...
					// If not official dictionary try to get the ID from http
					// custom server - if configured
					if (hasEntityType && (!adempiereSys)
						&& (!isExceptionCentralized (TableName)))
					{
						String isUseProjectCentralizedID = MSysConfig.getValue (
							"PROJECT_ID_USE_CENTRALIZED_ID", "N"); // defaults
						// to N
						if (isUseProjectCentralizedID.equals ("Y"))
						{
							...
							if (retValue > 0)
							{
								PreparedStatement updateSQL;
								updateSQL = conn
									.prepareStatement ("UPDATE AD_Sequence SET CurrentNext = GREATEST(CurrentNext, ? + 1) WHERE AD_Sequence_ID = ?");
								try
								{
									updateSQL.setInt (1, retValue);
									updateSQL.setInt (2, AD_Sequence_ID);
					...
					if (!gotFromHTTP)
					{
						...
						if (USE_PROCEDURE)
						{
							retValue = nextID (conn, AD_Sequence_ID,
								adempiereSys);
						}
						else
						{
			...
			{
				s_log
					.log (Level.SEVERE, TableName + " - " + e.getMessage (), e);
				try
				{
					if (conn != null)
						conn.rollback ();
	 ...
	 */
	private static int nextID(Connection conn, int AD_Sequence_ID,
	 ...
	 */
	public static synchronized String getDocumentNo(int AD_Client_ID,
		String TableName, String trxName)
	{
	 ...
	 */
	public static synchronized String getDocumentNo(int AD_Client_ID,
		String TableName, String trxName, PO po)
	{
		if (TableName == null || TableName.length () == 0)
			throw new IllegalArgumentException ("TableName missing");
		// get from server
		if (DB.isRemoteObjects ())
		{
			...
			try
			{
				if (server != null)
				{ // See ServerBean
					String dn = server.getDocumentNo (AD_Client_ID, TableName,
						trxName);
		...
		try
		{
			...
			//
			rs = pstmt.executeQuery ();
			// s_log.fine("AC=" + conn.getAutoCommit() + " -Iso=" +
			// conn.getTransactionIsolation()
			// + " - Type=" + pstmt.getResultSetType() + " - Concur=" +
			// pstmt.getResultSetConcurrency());
			if (rs.next ())
			{
				...
				if (USE_PROCEDURE)
				{
					next = isStartNewYear ? nextIDByYear (conn, AD_Sequence_ID,
						incrementNo, calendarYear) : nextID (conn,
						AD_Sequence_ID, adempiereSys);
				}
				else
				{
					...
					try
					{
						...
						{
							String sql = isStartNewYear ? "UPDATE AD_Sequence_No SET CurrentNext = CurrentNext + ? WHERE AD_Sequence_ID = ? AND CalendarYear = ?"
			...
			else
			{
				...
				MSequence seq = new MSequence (Env.getCtx (), AD_Client_ID,
					TableName, null);
				next = seq.getNextID ();
				seq.save ();
			}
		...
		s_log.finer (documentNo + " (" + incrementNo + ")" + " - Table="
			+ TableName + " [" + trx + "]");
		return documentNo;
	} // getDocumentNo
				...
				if (v != null)
				{
					if (format != null && format.length () > 0)
					{
						if (v instanceof Integer && token.endsWith ("_ID"))
						{
							...
							String table = tblIndex > 0 ? format.substring (0,
								tblIndex) : token.substring (0,
								token.length () - 3);
							String column = tblIndex > 0 ? format
								.substring (tblIndex + 1) : format;
							outStr.append (DB
								.getSQLValueString (trxName, "select " + column
						...
						else if (v instanceof Number)
						{
							...
							outStr.append (df.format (((Number)v)
								.doubleValue ()));
						}
						else
						{
	 ...
	 * @return document no or null
	 */
	public static synchronized String getDocumentNo(int C_DocType_ID,
		String trxName, boolean definite)
	{
	 ...
	 */
	public static synchronized String getDocumentNo(int C_DocType_ID,
		String trxName, boolean definite, PO po)
	{
		if (C_DocType_ID == 0)
		{
		...
		if (DB.isRemoteObjects ())
		{
			...
			try
			{
				...
				s_log.log (Level.SEVERE, "AppsServer not found - "
					+ C_DocType_ID);
			}
			catch (RemoteException ex)
			{
		...
		// local
		MDocType dt = MDocType.get (Env.getCtx (), C_DocType_ID); // wrong for
		// SERVER,
		// but r/o
		if (dt != null && !dt.isDocNoControlled ())
		{
			s_log
				.finer ("DocType_ID=" + C_DocType_ID + " Not DocNo controlled");
			return null;
		}
		if (definite && !dt.isOverwriteSeqOnComplete ())
		{
			s_log.finer ("DocType_ID=" + C_DocType_ID
				+ " Not Sequence Overwrite on Complete");
			return null;
		}
		...
		if (suffix != null && suffix.length () > 0)
			doc.append (parseVariable (suffix, po, trxName));
		String documentNo = doc.toString ();
		s_log.finer (documentNo + " (" + incrementNo + ")" + " - C_DocType_ID="
			+ C_DocType_ID + " [" + trx + "]");
		return documentNo;
	} // getDocumentNo
		...
		int AD_Client_ID, String trxName)
	{
		String sql = "SELECT TableName "
			...
			+ "(SELECT Name FROM AD_Sequence s " + "WHERE s.AD_Client_ID=?)";
		int counter = 0;
		...
		s_log.info ("AD_Client_ID=" + AD_Client_ID + " - created #" + counter
			+ " - success=" + success);
		return success;
	...
	{
		String sql = "SELECT * FROM AD_Sequence " + "WHERE UPPER(Name)=?"
			+ " AND IsTableID='Y'";
		...
		try
		{
			...
			if (rs.next ())
				s_log.log (Level.SEVERE, "More then one sequence for "
					+ tableName);
		}
		catch (Exception e)
		{
	...
	public static final int INIT_SYS_NO = 50000; // start number for
	 ...
	 * @param trxName trx
	 */
	public MSequence(Properties ctx, int AD_Client_ID, String sequenceName,
		int StartNo, String trxName)
	{
		this (ctx, 0, trxName);
		setClientOrg (AD_Client_ID, 0); // Client Ownership
	...
	public boolean validateTableIDValue()
	{
		...
		int AD_Column_ID = DB.getSQLValue (null, "SELECT MAX(c.AD_Column_ID) "
			...
			+ "WHERE t.TableName='" + tableName + "'" + " AND c.ColumnName='"
			+ tableName + "_ID'");
		if (AD_Column_ID <= 0)
			return false;
		//
		...
		sql = "SELECT MAX(" + tableName + "_ID) FROM " + tableName + " WHERE "
			+ tableName + "_ID < " + INIT_NO;
	...
	static public void main(String[] args)
	{
		 ...
		 * System.out.println(DB.getDocumentNo(118, trxName));
		 * System.out.println(DB.getDocumentNo(117, trxName)); trxName =
		 * "test1"; System.out.println(DB.getDocumentNo(115, trxName)); // hangs
		 * here as supposed System.out.println(DB.getDocumentNo(116, trxName));
		 * System.out.println(DB.getDocumentNo(117, trxName));
		 * System.out.println(DB.getDocumentNo(118, trxName)); /**
		 **********************************************************************/
		/** Time Test */
		...
		System.out.println ("Time (ms)=" + time + " - "
			+ ((float)time / s_list.size ()) + " each");
		System.out.println ("-------------------------------------------");
		/***********************************************************************
		 * try { int retValue = -1; Connection conn = DB.getConnectionRW (); //
		 * DriverManager.registerDriver (new oracle.jdbc.driver.OracleDriver()); //
		 * Connection conn = DriverManager.getConnection
		 * ("jdbc:oracle:thin:@//dev2:1521/dev2", "adempiere", "adempiere");
		 * conn.setAutoCommit(false); String sql = "SELECT CurrentNext,
		 * CurrentNextSys, IncrementNo " + "FROM AD_Sequence " + "WHERE
		 * Name='AD_Sequence' "; sql += "FOR UPDATE"; // creates ORA-00907:
		 * missing right parenthesis // sql += "FOR UPDATE OF CurrentNext,
		 * CurrentNextSys"; PreparedStatement pstmt = conn.prepareStatement(sql,
		 * ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE); ResultSet
		 * rs = pstmt.executeQuery(); System.out.println("AC=" +
		 * conn.getAutoCommit() + ", RO=" + conn.isReadOnly() + " - Isolation=" +
		 * conn.getTransactionIsolation() + "(" +
		 * Connection.TRANSACTION_READ_COMMITTED + ") - RSType=" +
		 * pstmt.getResultSetType() + "(" + ResultSet.TYPE_SCROLL_SENSITIVE +
		 * "), RSConcur=" + pstmt.getResultSetConcurrency() + "(" +
		 * ResultSet.CONCUR_UPDATABLE + ")"); if (rs.next()) { int IncrementNo =
		 * rs.getInt(3); retValue = rs.getInt(1); rs.updateInt(1, retValue +
		 * IncrementNo); rs.updateRow(); } else s_log.severe ("no record
		 * found"); rs.close(); pstmt.close(); conn.commit(); conn.close(); //
		 * System.out.println("Next=" + retValue); } catch (Exception e) {
		 * e.printStackTrace (); } System.exit(0); /** ** int AD_Client_ID = 0;
		 * int C_DocType_ID = 115; // GL String TableName = "C_Invoice"; String
		 * trxName = "x"; Trx trx = Trx.get(trxName, true); System.out.println
		 * ("none " + getNextID (0, "Test")); System.out.println
		 * ("----------------------------------------------");
		 * System.out.println ("trx1 " + getNextID (0, "Test"));
		 * System.out.println ("trx2 " + getNextID (0, "Test")); //
		 * trx.rollback(); System.out.println ("trx3 " + getNextID (0, "Test")); //
		 * trx.commit(); System.out.println ("trx4 " + getNextID (0, "Test")); //
		 * trx.rollback(); // trx.close(); System.out.println
		 * ("----------------------------------------------");
		 * System.out.println ("none " + getNextID (0, "Test"));
		 * System.out.println
		 * ("=============================================="); trx =
		 * Trx.get(trxName, true); System.out.println ("none " +
		 * getDocumentNo(AD_Client_ID, TableName, null)); System.out.println
		 * ("----------------------------------------------");
		 * System.out.println ("trx1 " + getDocumentNo(AD_Client_ID, TableName,
		 * trxName)); System.out.println ("trx2 " + getDocumentNo(AD_Client_ID,
		 * TableName, trxName)); trx.rollback(); System.out.println ("trx3 " +
		 * getDocumentNo(AD_Client_ID, TableName, trxName)); trx.commit();
		 * System.out.println ("trx4 " + getDocumentNo(AD_Client_ID, TableName,
		 * trxName)); trx.rollback(); trx.close(); System.out.println
		 * ("----------------------------------------------");
		 * System.out.println ("none " + getDocumentNo(AD_Client_ID, TableName,
		 * null)); System.out.println
		 * ("=============================================="); trx =
		 * Trx.get(trxName, true); System.out.println ("none " +
		 * getDocumentNo(C_DocType_ID, null)); System.out.println
		 * ("----------------------------------------------");
		 * System.out.println ("trx1 " + getDocumentNo(C_DocType_ID, trxName));
		 * System.out.println ("trx2 " + getDocumentNo(C_DocType_ID, trxName));
		 * trx.rollback(); System.out.println ("trx3 " +
		 * getDocumentNo(C_DocType_ID, trxName)); trx.commit();
	...
	public static synchronized int getNextOfficialID_HTTP(String TableName)
	{
		...
		return getNextID_HTTP (TableName, website, prm_USER, prm_PASSWORD,
			prm_TABLE, prm_ALTKEY, prm_COMMENT, prm_PROJECT);
	}
	...
	private static int getNextID_HTTP(String TableName, String website,
		String prm_USER, String prm_PASSWORD, String prm_TABLE,
		String prm_ALTKEY, String prm_COMMENT, String prm_PROJECT)
	{
		...
		try
		{
			...
			// its various parts: protocol, host, port, filename. Check the
			// protocol
		...
		s_log.log (Level.INFO, "getNextID_HTTP - " + TableName + "=" + read
			+ "(" + retValue + ")");
		return retValue;
	}
	...
	private static boolean isExceptionCentralized(String tableName)
	{
		...
		{"AD_ACCESSLOG", "AD_ALERTPROCESSORLOG", "AD_CHANGELOG", "AD_ISSUE",
			"AD_LDAPPROCESSORLOG", "AD_PACKAGE_IMP", "AD_PACKAGE_IMP_BACKUP",
			"AD_PACKAGE_IMP_DETAIL", "AD_PACKAGE_IMP_INST",
			...
			"T_INVENTORYVALUE", "T_INVOICEGL", "T_REPLENISH", "T_REPORT",
			"T_REPORTSTATEMENT", "T_SELECTION", "T_SELECTION2", "T_SPOOL",
			"T_TRANSACTION", "T_TRIALBALANCE"};
		for (int i = 0; i < exceptionTables.length; i++)
		{
			if (tableName.equalsIgnoreCase (exceptionTables[i]))
				return true;
		}
		// don't log selects or insert/update for exception tables (i.e.
		// AD_Issue, AD_ChangeLog)
		return false;
	}
	 ...
	 */
	public static String getPreliminaryNoByYear(GridTab tab,
		int AD_Sequence_ID, String dateColumn, String trxName)
	{
		...
		return DB
			.getSQLValueString (trxName, sql, AD_Sequence_ID, calendarYear);
	}
} // MSequence
