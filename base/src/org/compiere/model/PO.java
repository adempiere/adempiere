...
//				int index = p_info.getColumnIndex(column);
	...
	//	MField.getDefault(p_info.getDefaultLogic(i));
		...
		// Following the function getDefaults in GridField
		// but based on p_info instead of GridFieldVO
		...
		for (int i = 0; i < size; i++)
		{
			...
			/**
			 *  (a) Key/IsActive/Client/System/*atedBy/Dates/Table ID
			 */
	
			//	No defaults for these fields
			if (p_info.isKey(i) || p_info.getColumnDisplayType(i) == DisplayType.RowID 
				|| DisplayType.isLOB(p_info.getColumnDisplayType(i))) {
				m_newValues[i] = null;
			} 

			//	Always Active
			else if (p_info.getColumnName(i).equals("IsActive"))
			{
				m_newValues[i] = new Boolean(true);
			}
			
			//	Set Client to context, 
			else if (p_info.getColumnName(i).equals("AD_Client_ID"))
			{
				m_newValues[i] = new Integer(Env.getAD_Client_ID(p_ctx));
			}

			//	Set Org to context, 
			else if (p_info.getColumnName(i).equals("AD_Org_ID"))
			{
				m_newValues[i] = new Integer(Env.getAD_Client_ID(p_ctx));
			}
			//  Set Created and Updated By
			else if (p_info.getColumnName(i).endsWith("tedBy"))
			{
				m_newValues[i] = new Integer (Env.getContextAsInt(p_ctx, "#AD_User_ID"));
			}
			//  Set Created and Updated timestamp
			else if (p_info.getColumnName(i).equals("Created") || p_info.getColumnName(i).equals("Updated"))
			{
				m_newValues[i] = new Timestamp (System.currentTimeMillis());
			}
			// Set Processed
			else if (p_info.getColumnName(i).equals("Processed")) {
				m_newValues[i] = new Boolean(false);
			}
			// Set Processing
			else if (p_info.getColumnName(i).equals("Processing")) {
				m_newValues[i] = new Boolean(false);
			}
			// Set Posted
			else if (p_info.getColumnName(i).equals("Posted")) {
				m_newValues[i] = new Boolean(false);
			}
			//  Key column
			else if (p_info.getColumnName(i).equals(p_info.getTableName() + "_ID")) 
			{
				m_newValues[i] = I_ZERO;
			}
			
			/**
			 *  (b) SQL Statement (for data integrity & consistency)
			 */
			
			else if (p_info.getDefaultLogic(i) != null)  // Catch null pointer exception
			{
				if (p_info.getDefaultLogic(i) != null && p_info.getDefaultLogic(i).startsWith("@SQL="))
				{
					String sql = p_info.getDefaultLogic(i).substring(5);			//	w/o tag
					...
					sql = Env.parseContext(p_ctx, 0, sql, false, false);	//	replace variables
					if (sql.equals(""))
					{
						log.log(Level.WARNING, "(" + p_info.getColumnName(i) + ") - Default SQL variable parse failed: "
							+ p_info.getDefaultLogic(i));
					}
					else
					{
						try
						{
							...
							if (rs.next())
								...
								log.log(Level.WARNING, "(" + p_info.getColumnName(i) + ") - no Result: " + sql);
							rs.close();
							stmt.close();
						}
						catch (SQLException e)
						{
							log.log(Level.WARNING, "(" + p_info.getColumnName(i) + ") " + sql, e);
						}
					}
					if (defStr != null && defStr.length() > 0)
					{
						m_newValues[i] =  createDefault(p_info, i, defStr);
					}
				}	//	SQL Statement
				 ...
				 */
				else if (!p_info.getDefaultLogic(i).equals("") && !p_info.getDefaultLogic(i).startsWith("@SQL="))
				{
					defStr = "";		//	problem is with texts like 'sss;sss'
					//	It is one or more variables/constants
					StringTokenizer st = new StringTokenizer(p_info.getDefaultLogic(i), ",;", false);
					while (st.hasMoreTokens())
					{
						...
						if (defStr.equals("@SysDate@")) {				//	System Time
							...
							defStr = Env.getContext(p_ctx, 0, defStr.replace('@',' ').trim());
...
//			}
			...
			//	Button to N
			else if (p_info.getColumnDisplayType(i) == DisplayType.Button && !p_info.getColumnName(i).endsWith("_ID"))
			{
				m_newValues[i] = createDefault(p_info, i, "N");
			}
			//	CheckBoxes default to No - there are some YesNo displays that are strings/lists e.g. IsMandatory
			else if (p_info.getColumnDisplayType(i) == DisplayType.YesNo && p_info.getColumnClass(i) == Boolean.class )
			{
				m_newValues[i] = new Boolean(false);
			}
		...
		//	}
			//  IDs remain null
			else if (p_info.getColumnName(i).endsWith("_ID"))
			{
				m_newValues[i] = null;
			}
			//  actual Numbers default to zero
			else if (DisplayType.isNumeric(p_info.getColumnDisplayType(i)))
			{
				m_newValues[i] = createDefault(p_info, i, "0");
			}
			/**
			 *  No resolution
			 */
	...
	/**
	 *	Create Default Object type.  Taken from the Gridfield function of the same name.
	 *  <pre>
	 *		Integer 	(IDs, Integer)
	 *		BigDecimal 	(Numbers)
	 *		Timestamp	(Dates)
	 ...
	 */
	private Object createDefault (POInfo p_info, int i, String value)
	{
		//	true NULL
		...
		try
		{
			...
			if (p_info.getColumnName(i).endsWith("atedBy")
					|| (p_info.getColumnName(i).endsWith("_ID") && DisplayType.isID(p_info.getColumnDisplayType(i)))) // teo_sarca [ 1672725 ] Process parameter that ends with _ID but is boolean
			{
				try	//	defaults -1 => null
				{
			...
			//	Integer
			if (p_info.getColumnDisplayType(i) == DisplayType.Integer)
				return new Integer(value);
			
			//	Number
			if (DisplayType.isNumeric(p_info.getColumnDisplayType(i)))
				return new BigDecimal(value);
			
			//	Timestamps
			if (DisplayType.isDate(p_info.getColumnDisplayType(i)))
			{
			...
			//	Boolean
			if (p_info.getColumnDisplayType(i) == DisplayType.YesNo)
		...
		{
			log.log(Level.SEVERE, p_info.getColumnName(i) + " - " + e.getMessage());
		}
		return null;
	}	//	createDefault
