...
public class MDunningRunEntry extends X_C_DunningRunEntry
{
	...
	public List<MDunningRunLine> getLines (boolean onlyInvoices) {
		String whereClause = "C_DunningRunEntry_ID=?";
		if (onlyInvoices) {
			whereClause += " AND C_Invoice_ID IS NOT NULL";
		}
		//	
		return new Query(
				getCtx(),
				I_C_DunningRunLine.Table_Name,
				whereClause,
				get_TrxName())
	...
	public boolean hasInvoices() {
		...
		return DB.getSQLValue(get_TrxName(), sql, getC_DunningRunEntry_ID()) > 0;
	}	//	hasInvoices
