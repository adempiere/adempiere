 ...
 */
public class MCombinedAging extends X_T_CombinedAging
{
	/**
	 * 
	 */
	private static final long serialVersionUID = -4518497533256203256L;

	/**
	 * 	Standard Constructor
	 *	@param ctx context
	 *	@param T_CombinedAging_ID id
	 *	@param trxName transaction
	 */
	public MCombinedAging (Properties ctx, int T_CombinedAging_ID, String trxName)
	{
	 ...
	 *	@param trxName transaction
	 */
	public MCombinedAging (Properties ctx, int AD_PInstance_ID, Timestamp StatementDate, 
		int C_BPartner_ID, int C_Currency_ID, 
		int C_Invoice_ID, int C_Payment_ID, int C_InvoicePaySchedule_ID,
	 ...
	 *	@param trxName transaction
	 */
	public MCombinedAging (Properties ctx, ResultSet rs, String trxName)
	{
	 ...
	 *	@param trxAmt document transaction amount
	...
	public void add (Timestamp DueDate, int daysDue, BigDecimal invoicedAmt, BigDecimal openAmt)
	{
		...
		setTrxAmt(getTrxAmt().add(invoicedAmt));
		if (openAmt == null)
			openAmt = Env.ZERO;
	...
	{
		StringBuffer sb = new StringBuffer("MCombinedAging[");
