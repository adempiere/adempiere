...
public class ImportInventory extends ImportInventoryAbstract {
	...
	protected String doIt() throws java.lang.Exception {
		...
		sql = new StringBuffer ("UPDATE I_Inventory "
				...
				+ " ' || (SELECT COUNT(i.I_Inventory_ID) FROM I_Inventory i WHERE i.M_Product_ID = I_Inventory.M_Product_ID AND i.M_Locator_ID = I_Inventory.M_Locator_ID AND i.I_IsImported<>'Y' GROUP BY i.M_Locator_ID, i.M_Product_ID) "
				+ "WHERE EXISTS(SELECT 1 FROM I_Inventory i WHERE i.M_Product_ID = I_Inventory.M_Product_ID AND i.M_Locator_ID = I_Inventory.M_Locator_ID AND i.I_IsImported<>'Y' "
				+ "AND (i.M_AttributeSetInstance_ID = I_Inventory.M_AttributeSetInstance_ID OR i.M_AttributeSetInstance_ID IS NULL) "
				+ "GROUP BY i.M_Locator_ID, i.M_Product_ID, i.M_AttributeSetInstance_ID HAVING(COUNT(i.I_Inventory_ID) > 1)) "
				+ "AND EXISTS(SELECT 1 FROM M_Product p LEFT JOIN M_AttributeSet a ON(a.M_AttributeSet_ID = p.M_AttributeSet_ID) "
				+ "WHERE p.M_Product_ID = I_Inventory.M_Product_ID AND (p.M_AttributeSet_ID IS NULL OR a.IsInstanceAttribute = 'Y'))"
				+ " AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate (sql.toString (), get_TrxName());
		if (no != 0)
