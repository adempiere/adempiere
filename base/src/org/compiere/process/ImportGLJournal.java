	...
	protected String doIt() throws java.lang.Exception
	{
		...
		Trx.run((String trxName) -> {
				...
				ModelValidationEngine.get().fireImportValidate(this, null ,  null , ImportValidator.TIMING_BEFORE_VALIDATE);
							...
							sql = new StringBuilder("UPDATE I_GLJournal i "
									+ "SET CurrencyRate=? WHERE EXISTS (SELECT * FROM C_AcctSchema a "
									+ "WHERE a.C_AcctSchema_ID=i.C_AcctSchema_ID AND a.C_Currency_ID=i.C_Currency_ID) "
									+ "AND C_Currency_ID IS NOT NULL AND I_IsImported<>?").append(clientCheck);
							no = DB.executeUpdate(sql.toString(), new Object[] {1 , "Y"}, false , trxName);
