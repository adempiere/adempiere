...
public class ValuationEffectiveDate extends SvrProcess {
	...
	private StringBuffer whereClause1 = new StringBuffer(
			"WHERE tc.IsReversal='N' ");
	private StringBuffer whereClause2 = new StringBuffer(
			" AND  tc.SeqNo = (SELECT MAX(SeqNo) FROM RV_M_Transaction_Costing tc1")
		...
		for (MWarehouse warehouse : warehouses)
			for (MCostType costType : costTypes)
				for (MCostElement costElement : costElements)
					generateInventoryValue(warehouse.getM_Warehouse_ID(),
	...
	/**
	 * Define where clause based on process parameters
	 * 
	 * @param M_Warehouse_ID
	 * @param M_CostType_ID
	 * @param M_CostElement_ID
	 */
	public void setWhere(int M_Warehouse_ID, int M_CostType_ID,
		...
		if (M_CostElement_ID > 0) {
			...
			whereClause2
					.append(" AND tc1.M_CostElement_ID = tc.M_CostElement_ID");
		}
	 ...
	 */
	private void generateInventoryValue(int M_Warehouse_ID, int M_CostType_ID,
			int M_CostElement_ID) {
