...
public class BrowseElementHandler extends AbstractElementHandler {
		...
		if (isProcessElement(ctx, entitytype)) {
			...
			m_Browse.setAccessLevel (atts.getValue("AccessLevel"));
 			...
 			if (m_Browse.save(getTrxName(ctx)) == true) {
				record_log(ctx, 1, m_Browse.getName(), "Browse", m_Browse
						.get_ID(), AD_Backup_ID, Object_Status, "AD_Browse",
						get_IDWithColumn(ctx, "AD_Table", "TableName",
								"AD_Browse"));
				element.recordId = m_Browse.getAD_Browse_ID();
				browses.add(m_Browse.getAD_Browse_ID());
			} else {
				record_log(ctx, 0, m_Browse.getName(), "Browse", m_Browse
						.get_ID(), AD_Backup_ID, Object_Status, "AD_Browse",
						get_IDWithColumn(ctx, "AD_Table", "TableName",
								"AD_Browse"));
	...
	public void create(Properties ctx, TransformerHandler document)
		...
		List<MBrowseField> browseFields = new Query(ctx, I_AD_Browse_Field.Table_Name , whereClause.toString(), getTrxName(ctx))
		.setParameters(m_Browse.getAD_Browse_ID())
		.list();
	...
	private void createBrowseField(Properties ctx, TransformerHandler document,
		...
		Env.setContext(ctx, X_AD_Browse_Field.COLUMNNAME_AD_Browse_Field_ID, AD_Browse_Field_ID);
	...
	private AttributesImpl createBrowseBinding(AttributesImpl atts,
		...
		String name = DB.getSQLValueString(null, sql, m_Browse
				.getAD_Browse_ID());
		...
		atts.addAttribute("", "", "isBetaFunctionality", "CDATA", (m_Browse
