...
public class ViewColumnElementHandler extends AbstractElementHandler
{
	...
	{				
		...
		if (isProcessElement(ctx, entitytype)) {
			if (element.parent != null && element.parent.getElementValue().equals("viewdefinition") &&
			...
			int viewid = get_ID(ctx, "AD_View", atts
					.getValue("ADViewNameID"));
			if (viewid <= 0) {
				element.defer = true;
				return;
			}
			...
			if (element.parent != null && element.parent.getElementValue().equals("viewdefinition") &&
				...
				StringBuffer sqlB = new StringBuffer(
						"select AD_View_Definition_ID from AD_View_Definition where AD_View_ID = "
								+ viewid).append(" and TableAlias = '" + viewdenitionname + "'")
						.append(" and AD_Table_ID = ?");
				viewdefinitionid = DB.getSQLValue(getTrxName(ctx), sqlB.toString(),
						tableid);
			...
			if (viewdefinitionid > 0) {
				...
				int id = DB.getSQLValue(getTrxName(ctx), sqlB.toString(), columnid,viewdefinitionid);
				...
				m_ColumnView.setIsActive(atts.getValue("isActive") != null ? Boolean
						.valueOf(atts.getValue("isActive")).booleanValue()
						: true);							
				
				if (m_ColumnView.save(getTrxName(ctx)) == true) {
					record_log(ctx, 1, m_ColumnView.getName(), "ViewColumn", m_ColumnView
							.get_ID(), AD_Backup_ID, Object_Status, "ViewColumn",
							get_IDWithColumn(ctx, "AD_Table", "TableName",
									"AD_View_Column"));
					element.recordId = m_ColumnView.getAD_View_Column_ID();
				} else {
					record_log(ctx, 0, m_ColumnView.getName(), "ViewColumn", m_ColumnView
							.get_ID(), AD_Backup_ID, Object_Status, "AD_View_Column",
							get_IDWithColumn(ctx, "AD_Table", "TableName",
									"AD_View_Column"));
	...
	private AttributesImpl createViewColumnBinding(AttributesImpl atts,
		...
		if (m_ColumnView.getAD_View_Column_ID() <= PackOut.MAX_OFFICIAL_ID)
			atts.addAttribute("", "", "AD_View_Column_ID", "CDATA", Integer.toString(m_ColumnView.getAD_View_Column_ID()));
		if (m_ColumnView.getAD_Column_ID() > 0) {
			sql = "SELECT ColumnName FROM AD_Column WHERE AD_Column_ID=?";
			name = DB.getSQLValueString(null, sql, m_ColumnView.getAD_Column_ID());
		...
		if (m_ColumnView.getAD_Column_ID() > 0) {
			...
			int idTable = DB.getSQLValue(null, sql, m_ColumnView.getAD_Column_ID());
		...
		if (m_ColumnView.getAD_View_Column_ID() > 0) {
			...
			name = DB.getSQLValueString(null, sql, m_ColumnView.getAD_View_Column_ID());
		...
		if (m_ColumnView.getAD_View_Definition_ID() > 0) {
			...
			name = DB.getSQLValueString(null, sql, m_ColumnView.getAD_View_Definition_ID());
			...
			int viewid = DB.getSQLValue(null, sql, m_ColumnView.getAD_View_Definition_ID());
		...
		atts.addAttribute("", "", "Description", "CDATA",
				(m_ColumnView.getDescription() != null ? m_ColumnView.getDescription() : ""));
		
		atts.addAttribute("", "", "ColumnName", "CDATA",
				(m_ColumnView.getColumnName() != null ? m_ColumnView.getColumnName() : ""));
		atts.addAttribute("", "", "ColumnSQL", "CDATA",
				(m_ColumnView.getColumnSQL() != null ? m_ColumnView.getColumnSQL() : ""));
