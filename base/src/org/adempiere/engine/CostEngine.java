...
public class CostEngine {
	 ...
	 * @return
	 */
	public static BigDecimal getParentActualCostByCostType(MPPOrder order,
			MAcctSchema as, int M_CostType_ID, int M_CostElement_ID) {
		...
		whereClause.append(" cc.CostCollectorType <> '")
				.append(MPPCostCollector.COSTCOLLECTORTYPE_MaterialReceipt)
				.append("')");
	 ...
	 */
	public static BigDecimal getParentActualCostByCostType(
			X_M_ProductionLine line, MAcctSchema as, int M_CostType_ID,
	 ...
	 * @return MCostDetail
	 */
	private MCostDetail getCostDetail(IDocumentLine model, MTransaction mtrx,
			MAcctSchema as, int M_CostElement_ID) {
		final String whereClause = "AD_Client_ID=? AND AD_Org_ID=?" + " AND "
				+ model.get_TableName() + "_ID=?" + " AND "
		...
		final Object[] params = new Object[] { mtrx.getAD_Client_ID(),
				mtrx.getAD_Org_ID(), model.get_ID(), mtrx.getM_Product_ID(),
				mtrx.getM_AttributeSetInstance_ID(), as.getC_AcctSchema_ID(),
				// as.getM_CostType_ID(),
				M_CostElement_ID, };
	...
	public void createCostDetail(MTransaction mtrx, IDocumentLine model) {
		...
		for (MAcctSchema as : MAcctSchema.getClientAcctSchema(mtrx.getCtx(),
	...
	public void createCostDetail(MTransaction mtrx, IDocumentLine model,
			MAcctSchema as, Boolean isSOTrx) {
		MProduct product = new MProduct(mtrx.getCtx(), mtrx.getM_Product_ID(),
				mtrx.get_TrxName());
		String CostingLevel = product.getCostingLevel(as);
		// Org Element
		...
		List<MCostElement> ces = MCostElement.getCostElement(mtrx.getCtx(),
				mtrx.get_TrxName());
	 ...
	 *            Cost Element ID
	 */
	public void createCostDetail(MAcctSchema as, MTransaction mtrx,
			IDocumentLine model, MCostType ct, MCostElement ce) {
		...
		if (model instanceof MMatchInv
				&& MCostType.COSTINGMETHOD_AveragePO.equals(ct
						.getCostingMethod()))
			return;
		...
		if (model instanceof MMatchPO
				&& MCostType.COSTINGMETHOD_AverageInvoice.equals(ct
						.getCostingMethod()))
			return;
		...
		if ((MCostElement.COSTELEMENTTYPE_Material.equals(ce
				...
				&& !MCostType.COSTINGMETHOD_StandardCosting.equals(ct
						.getCostingMethod())) {
			if 
			...
			{					
				...
				if (model instanceof MMovementLine) {
					...
					BigDecimal costMovementFrom = getCostThisLevel(trx, as, ct, ce, model,costingLevel);
		...
		if (cd != null
				...
				&& MCostElement.COSTELEMENTTYPE_LandedCost.equals(ce
						.getCostElementType())) {
			MLandedCostAllocation allocation = (MLandedCostAllocation) model;
			cd.setM_InOutLine_ID(allocation.getM_InOutLine_ID());
			cd.setC_InvoiceLine_ID(allocation.getC_InvoiceLine_ID());
	 ...
	 *            Processed
	 */
	public void createCostDetail(IDocumentLine model, MAcctSchema as,
			CostComponent cc, Boolean isSOTrx, boolean setProcessed) {
		...
		int no = DB.executeUpdateEx(
				sql,
				new Object[] { model.get_ID(), as.getC_AcctSchema_ID(),
		...
		for (MCost cost : costs) {
			...
			final ICostingMethod method = CostingMethodFactory.get()
					.getCostingMethod(ce, cost.getCostingMethod());
			method.setCostingMethod(as, model, null, cost,
	...
	public void createCostDetailForLandedCostAllocation(
		...
		for (MTransaction trx : MTransaction.getByInOutLine(ioLine)) {
			for (MAcctSchema as : MAcctSchema.getClientAcctSchema(
				...
				List<MCostType> cts = MCostType.get(allocation.getCtx(),
						allocation.get_TrxName());

				for (MCostType ct : cts) {
					MCostElement ce = (MCostElement) allocation
							.getM_CostElement();
					CostEngineFactory.getCostEngine(
							allocation.getAD_Client_ID()).createCostDetail(as,
							trx, allocation, ct, ce);
				}
			}
		}
	}
	...
	public void createCostDetailForLandedCost(IDocumentLine model,
			MAcctSchema as, CostComponent cc, Boolean isSOTrx,
			boolean setProcessed) {
		final String idColumnName = model.get_TableName() + "_ID";
		final String trxName = model.get_TrxName();
		...
		for (MCost cost : costs) {
			...
			if (MCostElement.COSTELEMENTTYPE_LandedCost.equals(ce
					.getCostElementType())) {
				MCostDetail cd = new MCostDetail(as, model.getAD_Org_ID(),
						model.getM_Product_ID(),
						model.getM_AttributeSetInstance_ID(),
						ce.getM_CostElement_ID(), cc.getAmount(), cc.getQty(),
						ce.getDescription(), model.get_TrxName(),
				...
				cd.saveEx();
				if (!cd.isProcessed()) {
					MClient client = MClient.get(as.getCtx(),
							as.getAD_Client_ID());
	...
	public MCostDetail getCostDetail(MCost cost, IDocumentLine line,
		...
		String whereClauseFinal = idColumnName + "=?"
				+ " AND M_AttributeSetInstance_ID=?" + " AND C_AcctSchema_ID=?"
				+ " AND M_CostType_ID=?";
	...
	public void createStandardCostDetail(IDocumentLine model, MTransaction mtrx) {
		...
		for (MAcctSchema as : getAcctSchema(mtrx)) {
			...
			for (MCostElement element : getCostElements(mtrx.getCtx())) {
				//
				// Delete Unprocessed zero Differences
				deleteCostDetail(model, as, element.get_ID(),
						mtrx.getM_AttributeSetInstance_ID());
				//
				// Get Costs
				...
				final BigDecimal amt = roundCost(price.multiply(qty),
						as.getC_AcctSchema_ID());
				//
				// Create / Update Cost Detail
				MCostDetail cd = getCostDetail(model, mtrx, as,
						element.get_ID());
	...
	private int deleteCostDetail(IDocumentLine model, MAcctSchema as,
			int M_CostElement_ID, int M_AttributeSetInstance_ID) {
		// Delete Unprocessed zero Differences
		String sql = "DELETE "
		...
		Object[] parameters = new Object[] { model.get_ID(),
				as.getC_AcctSchema_ID(), M_AttributeSetInstance_ID,
				// as.getM_CostType_ID(),
				M_CostElement_ID };
	...
	private void processCostDetail(MCostDetail cd) {
		if (!cd.isProcessed()) {
			MAcctSchema as = MAcctSchema.get(cd.getCtx(),
					cd.getC_AcctSchema_ID());
	...
	private MPPCostCollector createVarianceCostCollector(MPPCostCollector cc,
	 ...
	 * @return
	 */
	private MCostDetail createVarianceCostDetail(MPPCostCollector ccv,
	...
	public void createRateVariances(MPPCostCollector cc) {
		...
		for (MAcctSchema as : getAcctSchema(cc)) {
			for (MCostElement element : getCostElements(cc.getCtx())) {
				final MCostDetail cd = getCostDetail(cc,
						element.getM_CostElement_ID());
				if (cd == null)
					continue;
				//
				...
				final BigDecimal amtStd = roundCost(priceStd.multiply(qty),
						as.getC_AcctSchema_ID());
				final BigDecimal amtActual = roundCost(
						priceActual.multiply(qty), as.getC_AcctSchema_ID());
				if (amtStd.compareTo(amtActual) == 0)
					continue;
				//
				if (ccrv == null) {
					ccrv = createVarianceCostCollector(cc,
	...
	public void createMethodVariances(MPPCostCollector cc) {
		...
		for (MAcctSchema as : getAcctSchema(cc)) {
			for (MCostElement element : getCostElements(cc.getCtx())) {
				...
				//
				if (ccmv == null) {
					ccmv = createVarianceCostCollector(
							cc,
	 ...
	 */
	public static BigDecimal getCostThisLevel(MTransaction mtrx, MAcctSchema as,
			I_M_CostType ct, I_M_CostElement ce, IDocumentLine model,
			String costingLevel) {
		BigDecimal costThisLevel = Env.ZERO;
		MCostDetail m_last_costdetail = MCostDetail.getLastTransaction(model,
				mtrx, as.getC_AcctSchema_ID(), ct.getM_CostType_ID(),
				ce.getM_CostElement_ID(), model.getDateAcct(), costingLevel);
		if (m_last_costdetail != null) {
				...
				{
					costThisLevel =  m_last_costdetail.getCostAmt().add(
				...
				else if (m_last_costdetail.getCumulatedQty().add( m_last_costdetail.getQty()).signum() != 0)
				{
					costThisLevel =  m_last_costdetail.getCostAmt().add(
									 m_last_costdetail.getCostAdjustment()).add(
									 m_last_costdetail.getCumulatedAmt())
							.divide(m_last_costdetail.getCumulatedQty().add( m_last_costdetail.getQty()), 
									as.getCostingPrecision(),
							BigDecimal.ROUND_HALF_UP).abs();
				...
				else if (m_last_costdetail.getCumulatedQty().signum() != 0)
				{
					costThisLevel = m_last_costdetail.getCumulatedAmt()
							.divide(m_last_costdetail.getCumulatedQty(), 
									as.getCostingPrecision(),
							BigDecimal.ROUND_HALF_UP).abs();
