...
public class EMailProcessor
{
	 ...
	 * @param password
	 */
	public EMailProcessor (String host, int port, String protocol, String encryptionType, 
			String authMechanism, String user, String password) {
		m_email = new EMail(host, port, protocol, encryptionType, authMechanism);
		m_email.createAuthenticator(user, password);
	}	//	EMail
	...
	private Store getStore() throws Exception {
		...
		//	Get Store
		//TODO Marek Mosiewicz - it seems for me that it will only work with imap
		m_store = m_session.getStore(m_email.getStringProtocol());
		//	Connect
		m_store.connect();
		//
...
//			Message[] deleted = inbox.expunge();
	...
	public static void main (String[] args) {
		EMailProcessor m = new EMailProcessor("admin", 110, 
				MEMailConfig.PROTOCOL_POP3, MEMailConfig.ENCRYPTIONTYPE_None, 
				MEMailConfig.AUTHMECHANISM_Login, "test", "testadempiere");
		m.processMessages();
	}	//	main
