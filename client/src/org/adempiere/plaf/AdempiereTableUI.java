...
/*
 * This package is copied from the BasicTableUI.java code and modified to provide
 * functionality to support frozen columns and the transfer of focus between
 * the frozen columns and the main table.  Access to the methods involved in the
 * BasicTableUI making these modifications necessary.
 */
...
/**
 *  ADempiere Table UI.
 *  Overrides specific hidden behavior in the BasicTableUI to provide
 *  support to frozen columns in VFrozenColumnRenderer.java
 *
 *	Original code
 *  @author Philip Milne
 *  @author Shannon Hickey (drag and drop)
 *  
 *  Modifications for frozen columns
 *  @author Michael McKay, mckayERP@gmail.com
 *  
 *  @version 3.9.4
 */
public class AdempiereTableUI extends TableUI
{

	static CLogger log = CLogger.getCLogger(AdempiereTableUI.class);
	...
	// The CTable that is delegating the painting to this UI.
	...
	private static class Actions extends UIAction {
		...
		private void moveWithinTableRange(JTable table, int dx, int dy) {
			...
			if (checkTableRangeAndSwitchFromToFrozenColumns(table, dx, dy))
			{
		...
		private boolean moveWithinSelectedRange(JTable table, int dx, int dy,
			...
			// The switch between tables does not support extended ranges.
			if (checkSelectedRangeAndSwitchFromToFrozenColumns(table, dx, minX, maxX, minY, maxY))
			{
			...
			if (key == NEXT_ROW ||  // Navigate Actions
				...
				if (th != null) {
					...
					if (col >= 0) {
						...
						if (thUI instanceof AdempiereTableHeaderUI) {
							((AdempiereTableHeaderUI)thUI).selectColumnImp(col);
						}
					}
		...
		/**
		 * Called to check if the movement should transition to or from
		 * the frozen columns, following the model of the selected range. This
		 * wraps the selection from first to last cell of the previous row or 
		 * last to first cell of the next row, and also from the last row to the first
		 * and vice versa.
		 * @param table the table holding the focus. This could be either the main table
		 * or the frozenColumnRenderer.
		 * @param dx - the current column displacement, either -1, 0, or 1
		 * @param dy - the current row displacement, either -1, 0 or 1
		 * @return true if the selection is staying in the current table or false
		 * if there has been a switch.
		 */
		private boolean checkTableRangeAndSwitchFromToFrozenColumns (JTable table, int dx, int dy) {
			...
			if (isVTable)
			{
				...
				else 
				{
					...
					if (frozenColumnRenderer != null && dx < 0 && leadColumn+dx < 0)
					{
						...
						if (toLimit)
							lastColumn = 0;
						frozenColumnRenderer.changeSelection(leadRow, lastColumn, false, false);
						//  No need to scroll
						frozenColumnRenderer.requestFocus();
						return false;
					}
				}
			} // isVtable
		...
		} // checkTableRangeAndSwitchFromToFrozenColumns
		 ...
		 * @param table the table holding the focus. This could be either the main table
		 * or the frozenColumnRenderer.
		 * @param dx - the current column displacement, either -1, , or 1;
		 * @param minX the table view minimum column position
		 * @param maxX the table view maximum column position
		 * @param minY - the minimum row
		 * @param maxY - the maximum row
		 * @return true if the selection is staying in the current table or false
		 * if there has been a switch.
		 */
		private boolean checkSelectedRangeAndSwitchFromToFrozenColumns(JTable table, int dx, int minX,
...
//							targetCol = mainTable.getColumnCount()-1;
				...
				else if (leadColumn+dx < minX) // Move to last cell on previous row
				{
...
//						targetCol = 0;
			...
			else 
			{
				...
				if (frozenColumnRenderer != null)
				{
					if (leadColumn+dx > maxX)
					{
						...
						frozenColumnRenderer.changeSelection(leadRow, targetColumn, false, false);
						//  No need to scroll
						frozenColumnRenderer.requestFocus();
						return false;
					}
					else if (leadColumn + dx < minX) 
					{
						// Stay on the row but move to the frozen columns
						int targetColumn = frozenColumnRenderer.getColumnCount()-1;
						frozenColumnRenderer.changeSelection(leadRow, targetColumn, false, false);						
						//  No need to scroll
						frozenColumnRenderer.requestFocus();
						return false;
	 ...
	 * Instantiate it only within subclasses of {@code AdempierTableUI}.
	...
	/**
	 * This class should be treated as a &quot;protected&quot; inner class.
	 * Instantiate it only within subclasses of AdempierTableUI.
	 */
	...
	BeforeDrag {
	     ...
	     */
	    private Component getDeepestComponentAt(Component parent, int x, int y, Component exclude) {
	        ...
	        if (parent instanceof Container) {
	            ...
	            for (Component comp : components) {
	                if (comp != null && comp.isVisible() && !(exclude!= null && comp.equals(exclude))) {
	                    Point loc = comp.getLocation();
	                    if (comp instanceof Container) {
	                        comp = getDeepestComponentAt(comp, x - loc.x, y - loc.y, exclude);
	                    } else {
	...
	protected void installKeyboardActions() {
		LazyActionMap.installLazyActionMap(table, AdempiereTableUI.class,
				"Table.actionMap");
	...
	static class TableTransferHandler extends TransferHandler implements UIResource {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 5057786992446426366L;
