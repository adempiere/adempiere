...
--************************************************************************
--  $Id: Structure_view.sql,v 1.1 2006/10/02 19:49:59 jjanke Exp $
-- **
--  Title:	Views
--  Description:	
...
-- 
--
--   09-28-06 Jinglun Zhang       create view sqls, modified from structure-1.sql
--
...
; CREATE VIEW C_Invoice_Header_v 
AS
...
; CREATE VIEW C_Invoice_Header_vt 
AS
SELECT i.AD_Client_ID, i.AD_Org_ID, i.IsActive, i.Created, i.CreatedBy, i.Updated, i.UpdatedBy, 
	dt.AD_Language,
	i.C_Invoice_ID, i.IsSOTrx, i.DocumentNo, i.DocStatus,  i.C_DocType_ID,
...
(
	...
	AD_Language,
	C_Invoice_ID, C_InvoiceLine_ID, 
    C_Tax_ID, TaxAmt, LineTotalAmt, TaxIndicator,
	Line, 
	QtyInvoiced, 
    QtyEntered, 
    UOMSymbol, 
	...
	ResourceDescription, -- forth line
	PriceList,
	PriceEnteredList,
	Discount,
	PriceActual, 
    ...
    SerNo, Lot, M_Lot_ID,GuaranteeDate, 
...
SELECT il.AD_Client_ID, il.AD_Org_ID, il.IsActive, il.Created, il.CreatedBy, il.Updated, il.UpdatedBy,
	'en_US', 
	il.	C_Invoice_ID, il.C_InvoiceLine_ID, 
    nullif(1,1), nullif(1,1), nullif(1,1), nullif('a','a'),
	il.Line,
	nullif(1,1), 
	nullif(1,1), 
	nullif('a','a'),
	...
	il.Description,
	nullif('a','a'), 
	nullif('a','a'), nullif('a','a'), nullif('a','a'), 
	nullif('a','a'), 
	nullif(1,1),nullif(1,1), nullif(1,1), nullif(1,1), nullif(1,1), nullif(1,1), 
	nullif(1,1), nullif(1,1), 
	nullif('a','a'), nullif('a','a'), nullif(1,1), nullif('a','a'), --last one timestamp
...
SELECT AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
	...
	nullif('a','a'),
	nullif('a','a'), nullif('a','a'), 
	nullif('a','a'), nullif('a','a'), nullif('a','a'), nullif('a','a'),
	nullif(1,1), nullif(1,1), nullif(1,1), nullif(1,1), nullif(1,1), nullif(1,1),
    nullif(1,1), nullif(1,1), 
    nullif('a','a'), nullif('a','a'), nullif(1,1), nullif('a','a'), 
    nullif('a','a'), nullif('a','a'),
    nullif(1,1), nullif(1,1), nullif(1,1), nullif(1,1), nullif(1,1)
FROM C_Invoice
UNION   --   tax lines
SELECT it.AD_Client_ID, it.AD_Org_ID, it.IsActive, it.Created, it.CreatedBy, it.Updated, it.UpdatedBy,
    ...
    CASE WHEN it.IsTaxIncluded='Y' THEN it.TaxAmt ELSE it.TaxBaseAmt END, 
    CASE WHEN it.IsTaxIncluded='Y' THEN nullif(1,1) ELSE it.TaxAmt END,
    nullif(1,1), nullif(1,1), 
    nullif('a','a'), nullif('a','a'), nullif(1,1), nullif('a','a'), 
    nullif('a','a'), nullif('a','a'),
    nullif(1,1), nullif(1,1), nullif(1,1), nullif(1,1), nullif(1,1)
...
SELECT il.AD_Client_ID, il.AD_Org_ID, il.IsActive, il.Created, il.CreatedBy, il.Updated, il.UpdatedBy,
	...
	nullif(1,1),nullif(1,1), nullif(1,1),	nullif(1,1), nullif(1,1), nullif(1,1), 
	nullif(1,1), nullif(1,1), 
	nullif('a','a'), nullif('a','a'), nullif(1,1), nullif('a','a'), --timestamp
	nullif('a','a'), nullif('a','a'), 
    nullif(1,1),nullif(1,1),nullif(1,1),nullif(1,1),nullif(1,1)
...
SELECT it.AD_Client_ID, it.AD_Org_ID, it.IsActive, it.Created, it.CreatedBy, it.Updated, it.UpdatedBy,
	...
	nullif('a','a'),
	t.Name,
	nullif('a','a'), nullif('a','a'), nullif('a','a'), nullif('a','a'), nullif('a','a'), nullif('a','a'),
	nullif(1,1), nullif(1,1), nullif(1,1),
    CASE WHEN it.IsTaxIncluded='Y' THEN it.TaxAmt ELSE it.TaxBaseAmt END, 
    CASE WHEN it.IsTaxIncluded='Y' THEN it.TaxAmt ELSE it.TaxBaseAmt END, 
    CASE WHEN it.IsTaxIncluded='Y' THEN nullif(1,1) ELSE it.TaxAmt END,
...
SELECT ol.AD_Client_ID, ol.AD_Org_ID, ol.IsActive, ol.Created, ol.CreatedBy, ol.Updated, ol.UpdatedBy,
	...
	CASE WHEN ol.PriceActual<>0 OR ol.M_Product_ID IS NOT NULL THEN ol.PriceActual else 0 END AS  PriceActual, 
	CASE WHEN ol.PriceEntered<>0 OR ol.M_Product_ID IS NOT NULL THEN ol.PriceEntered else 0 END AS  PriceEntered, 
	CASE WHEN ol.LineNetAmt<>0 OR ol.M_Product_ID IS NOT NULL THEN ol.LineNetAmt else 0 END AS  LineNetAmt,
    p.Description as ProductDescription, p.ImageURL,
...
SELECT ol.AD_Client_ID, ol.AD_Org_ID, ol.IsActive, ol.Created, ol.CreatedBy, ol.Updated, ol.UpdatedBy,
	'en_US' AS AD_Language,
	ol.C_Order_ID, ol.C_OrderLine_ID, ol.C_Tax_ID, nullif('a','a'),
    nullif(1,1), nullif(1,1), nullif('a','a'), nullif(1,1),
...
SELECT AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
	'en_US', 
	C_Order_ID, nullif(1,1), nullif(1,1), nullif('a','a'),
	nullif(1,1), nullif(1,1), nullif('a','a'), nullif(1,1),
    nullif(1,1), 
	nullif(1,1), nullif(1,1), nullif('a','a'),
	nullif('a','a'),
	nullif('a','a'), 
	nullif('a','a'), nullif('a','a'), nullif('a','a'), nullif('a','a'), 
	nullif('a','a'),
...
SELECT ot.AD_Client_ID, ot.AD_Org_ID, ot.IsActive, ot.Created, ot.CreatedBy, ot.Updated, ot.UpdatedBy,
	'en_US', 
	ot.C_Order_ID, nullif(1,1), ot.C_Tax_ID, t.TaxIndicator,
    nullif(1,1), nullif(1,1), nullif('a','a'), nullif(1,1),
	nullif(1,1),
	nullif(1,1), nullif(1,1), nullif('a','a'),
	t.Name,
...
(AD_Client_ID, AD_Org_ID, IsActive, Created, CreatedBy, Updated, UpdatedBy,
	AD_Language,
	C_Order_ID, C_OrderLine_ID, C_Tax_ID, TaxIndicator,
    	C_BPartner_ID, C_BPartner_Location_ID, BPName, C_Location_ID,
	Line, 	
	QtyOrdered,
	QtyEntered, 
	UOMSymbol,
	...
	PriceActual, 
	PriceEntered, 
	LineNetAmt,    ProductDescription, ImageURL,
    C_Campaign_ID, C_Project_ID, C_Activity_ID, C_ProjectPhase_ID, C_ProjectTask_ID
)AS
SELECT ol.AD_Client_ID, ol.AD_Org_ID, ol.IsActive, ol.Created, ol.CreatedBy, ol.Updated, ol.UpdatedBy,
	uom.AD_Language,
...
SELECT ol.AD_Client_ID, ol.AD_Org_ID, ol.IsActive, ol.Created, ol.CreatedBy, ol.Updated, ol.UpdatedBy,
	...
	nullif('a','a'), nullif(1,1), nullif(1,1), nullif(1,1), nullif(1,1), nullif(1,1), nullif(1,1), pt.Description AS ProductDescription, p.ImageURL,
    ol.C_Campaign_ID, ol.C_Project_ID, ol.C_Activity_ID, ol.C_ProjectPhase_ID, ol.C_ProjectTask_ID
FROM M_Product_BOM b	-- BOM lines
	INNER JOIN C_OrderLine ol ON (b.M_Product_ID=ol.M_Product_ID)
	INNER JOIN M_Product bp ON (bp.M_Product_ID=ol.M_Product_ID -- BOM Product
...
UNION
SELECT o.AD_Client_ID, o.AD_Org_ID, o.IsActive, o.Created, o.CreatedBy, o.Updated, o.UpdatedBy,
	l.AD_Language, o.C_Order_ID, nullif(1,1), nullif(1,1), nullif('a','a'),
	nullif(1,1),
	nullif(1,1), nullif('a','a'), nullif(1,1),
    nullif(1,1), nullif(1,1), nullif(1,1), nullif('a','a'),
	nullif('a','a'),
	nullif('a','a'), --Desc
	nullif('a','a'), nullif('a','a'), nullif('a','a'), nullif('a','a'), nullif('a','a'),
	nullif(1,1), nullif(1,1), nullif(1,1), nullif(1,1), nullif(1,1), nullif(1,1), nullif('a','a'), nullif('a','a'),
    nullif(1,1),nullif(1,1),nullif(1,1),nullif(1,1),nullif(1,1)
FROM C_Order o, AD_Language l
WHERE l.IsBaseLanguage='N' AND l.IsSystemLanguage='Y'
UNION
SELECT ot.AD_Client_ID, ot.AD_Org_ID, ot.IsActive, ot.Created, ot.CreatedBy, ot.Updated, ot.UpdatedBy,
	t.AD_Language, ot.C_Order_ID, nullif(1,1), ot.C_Tax_ID, t.TaxIndicator,
    nullif(1,1), nullif(1,1), nullif('a','a'), nullif(1,1),
	nullif(1,1),
	nullif(1,1), nullif(1,1), nullif('a','a'),
	t.Name,
...
; CREATE VIEW C_PaySelection_Check_v 
AS
...
; CREATE VIEW C_PaySelection_Check_vt 
AS
SELECT psc.AD_Client_ID, psc.AD_Org_ID, 
	l.AD_Language,
	psc.C_PaySelection_ID, psc.C_PaySelectionCheck_ID,
...
; CREATE VIEW C_PaySelection_Remittance_v 
AS
...
; CREATE VIEW C_Project_Header_v 
AS
...
; CREATE VIEW C_Project_Header_vt 
AS
SELECT p.AD_Client_ID, p.AD_Org_ID, p.IsActive, p.Created, p.CreatedBy, p.Updated, p.UpdatedBy, 
...
; CREATE VIEW C_Project_Details_v 
AS
...
; CREATE VIEW C_Project_Details_vt 
AS
SELECT pl.AD_Client_ID, pl.AD_Org_ID, pl.IsActive, pl.Created, pl.CreatedBy, pl.Updated, pl.UpdatedBy,
	l.AD_Language,
    pj.C_Project_ID, pl.C_ProjectLine_ID,
...
; CREATE VIEW C_RfQResponseLine_v
AS
...
; CREATE VIEW C_RfQResponseLine_vt
AS
SELECT rrl.C_RfQResponse_ID, rrl.C_RfQResponseLine_ID, rrl.C_RfQLine_ID, 
    rq.C_RfQResponseLineQty_ID, rq.C_RfQLineQty_ID,
    rrl.AD_Client_ID, rrl.AD_Org_ID, rrl.IsActive, rrl.Created, rrl.CreatedBy, rrl.Updated, rrl.UpdatedBy,
...
; CREATE VIEW C_RfQResponseLineQty_v
AS
...
--
DROP VIEW M_InOutConfirm_vt 
; CREATE VIEW M_InOutConfirm_vt 
AS
SELECT ioc.AD_Client_ID, ioc.AD_Org_ID, ioc.IsActive, ioc.Created, ioc.CreatedBy, ioc.Updated, ioc.UpdatedBy,
  dt.AD_Language,
  ioc.M_InOutConfirm_ID,
...
; CREATE VIEW M_InOut_LineConfirm_vt 
AS
SELECT iolc.AD_Client_ID, iolc.AD_Org_ID, iolc.IsActive, iolc.Created, iolc.CreatedBy, iolc.Updated, iolc.UpdatedBy,
	uom.AD_Language,
    iolc.M_InOutLineConfirm_ID, iolc.M_InOutConfirm_ID,
...
; CREATE VIEW C_Dunning_Header_v 
AS
...
; 
DROP VIEW RV_C_Invoice_ProductQtr
; 
DROP VIEW RV_C_Invoice_VendorMonth 
; 
DROP VIEW RV_C_Invoice_ProdMonth 
; 
DROP VIEW RV_C_Invoice_ProductMonth 
; 
...
; 
DROP VIEW Temp_RV_C_Invoice_Day 
; 
...
AS SELECT 
	...
	CASE WHEN PriceList=0 THEN 0 ELSE
	  CAST((PriceList-PriceActual)/PriceList*100 AS DECIMAL(15,5)) END AS Discount,
	CASE WHEN PriceLimit=0 THEN 0 ELSE
	  CAST((PriceActual-PriceLimit)/PriceLimit*100 AS DECIMAL(15,5)) END AS Margin,
	CASE WHEN PriceLimit=0 THEN 0 ELSE
	  (PriceActual-PriceLimit)*QtyInvoiced END AS MarginAmt,
	--	Line Amounts
	CAST(i.Multiplier*LineNetAmt AS DECIMAL(15,5)) AS LineNetAmt,
	CAST(i.Multiplier*PriceList*QtyInvoiced AS DECIMAL(15,5)) AS LineListAmt,
	CASE WHEN COALESCE(il.PriceLimit, 0)=0 THEN CAST(i.Multiplier*LineNetAmt AS DECIMAL(15,5)) ELSE
		CAST(i.Multiplier*PriceLimit*QtyInvoiced AS DECIMAL(15,5)) END AS LineLimitAmt,
	CAST(i.Multiplier*PriceList*QtyInvoiced-LineNetAmt AS DECIMAL(15,5)) AS LineDiscountAmt,
	CASE WHEN COALESCE(il.PriceLimit,0)=0 THEN 0 ELSE
		CAST(i.Multiplier*LineNetAmt-PriceLimit*QtyInvoiced AS DECIMAL(15,5)) END AS LineOverLimitAmt
FROM  RV_C_Invoice i
...
--	Invoice By Day
-- jz group by + func + sum issues


CREATE VIEW Temp_RV_C_Invoice_Day 
AS
SELECT AD_Client_ID, AD_Org_ID, SalesRep_ID,
	C_BPartner_ID, M_Product_Category_ID, M_Product_ID,
	firstOf(DateInvoiced, 'DD') AS DD_DateInvoiced,	--	DD Day, DY Week, MM Month
	firstOf(DateInvoiced, 'DY') AS DY_DateInvoiced,	--	DD Day, DY Week, MM Month
	firstOf(DateInvoiced, 'MM') AS MM_DateInvoiced,	--	DD Day, DY Week, MM Month
	firstOf(DateInvoiced, 'Q') AS Q_DateInvoiced,	--	Q Quarter
	LineNetAmt,
	LineListAmt,
	LineLimitAmt,
	LineDiscountAmt,
...
AS
SELECT AD_Client_ID, AD_Org_ID, SalesRep_ID,
	DD_DateInvoiced as DateInvoiced,	--	DD Day, DY Week, MM Month
	SUM(LineNetAmt) AS LineNetAmt,
	SUM(LineListAmt) AS LineListAmt,
	SUM(LineLimitAmt) LineLimitAmt,
	...
	CASE WHEN SUM(LineNetAmt)=0 THEN 0 ELSE
	  100-CAST((SUM(LineNetAmt)-SUM(LineOverLimitAmt))/SUM(LineNetAmt)*100 AS DECIMAL(15,5)) END AS LineOverLimit,
    	IsSOTrx
FROM Temp_RV_C_Invoice_Day
GROUP BY AD_Client_ID, AD_Org_ID, SalesRep_ID,
	DD_DateInvoiced, IsSOTrx
;
...
AS
SELECT AD_Client_ID, AD_Org_ID, SalesRep_ID,
	DY_DateInvoiced AS DateInvoiced,	--	DD Day, DY Week, MM Month
	SUM(LineNetAmt) AS LineNetAmt,
	SUM(LineListAmt) AS LineListAmt,
	SUM(LineLimitAmt) AS LineLimitAmt,
	SUM(LineDiscountAmt) AS LineDiscountAmt,
...
--CREATE VIEW RV_C_Invoice_Month
--AS
--SELECT AD_Client_ID, AD_Org_ID, SalesRep_ID,
...
AS
SELECT AD_Client_ID, AD_Org_ID, SalesRep_ID,
	MM_DateInvoiced AS DateInvoiced,	--	DD Day, DY Week, MM Month
	SUM(LineNetAmt) AS LineNetAmt,
	SUM(LineListAmt) AS LineListAmt,
	SUM(LineLimitAmt) AS LineLimitAmt,
	SUM(LineDiscountAmt) AS LineDiscountAmt,
...
FROM Temp_RV_C_Invoice_Day
GROUP BY AD_Client_ID, AD_Org_ID, SalesRep_ID,
	MM_DateInvoiced, IsSOTrx
;

--	Invoice By Customer and Quarter


-- CREATE VIEW RV_C_Invoice_CustomerProdQtr 
--AS
...
SELECT il.AD_Client_ID, il.AD_Org_ID,
	il.C_BPartner_ID, il.M_Product_Category_ID,
	il.Q_DateInvoiced AS DateInvoiced,	--	DD Day, DY Week, MM Month
	SUM(LineNetAmt) AS LineNetAmt,
	SUM(LineListAmt) AS LineListAmt,
	SUM(LineLimitAmt) AS LineLimitAmt,
	SUM(LineDiscountAmt) AS LineDiscountAmt,
...
--	firstOf(il.DateInvoiced, 'MM') AS DateInvoiced,	--	DD Day, DY Week, MM Month
	il.MM_DateInvoiced AS DateInvoiced,	--	DD Day, DY Week, MM Month
	SUM(LineNetAmt) AS LineNetAmt,
	SUM(LineListAmt) AS LineListAmt,
	SUM(LineLimitAmt) AS LineLimitAmt,
	SUM(LineDiscountAmt) AS LineDiscountAmt,
...
; CREATE VIEW RV_M_Transaction_Sum
AS
...
SELECT i.AD_Org_ID, i.AD_Client_ID, 
    i.DocumentNo, i.C_Invoice_ID, i.C_Order_ID, i.C_BPartner_ID, i.IsSOTrx,
	i.DateInvoiced,
    getDaysBetween(ips.DueDate,i.DateInvoiced) AS NetDays,
    ips.DueDate,
    getDaysBetween(getdate(),ips.DueDate) AS DaysDue,
    ips.DiscountDate, 
    ips.DiscountAmt,
...
; CREATE VIEW RV_Product_Costing
AS
...
AS
SELECT 	c.AD_Client_ID, c.AD_Org_ID, c.IsActive, 0 AS CreatedBy,CURRENT_TIMESTAMP AS Created,0 AS UpdatedBy,CURRENT_TIMESTAMP AS Updated,
...
; 
DROP VIEW temp_RV_Asset_SumMonth
; 

-- jz group by with func problem
...
CREATE VIEW temp_RV_Asset_SumMonth
AS
SELECT a.AD_Client_ID, a.AD_Org_ID, a.IsActive, a.Created, a.CreatedBy, a.Updated, a.UpdatedBy,
  a.A_Asset_ID, a.A_Asset_Group_ID, a.M_Product_ID, 
  a.Value, a.Name, a.Description, a.Help,
...
FROM C_ProjectLine l
    LEFT OUTER JOIN C_ProjectIssue i ON (i.C_Project_ID=l.C_Project_ID AND i.C_ProjectIssue_ID=l.C_ProjectIssue_ID)
...
SELECT COALESCE(l.AD_Client_ID,i.AD_Client_ID) AS AD_Client_ID,COALESCE(l.AD_Org_ID,i.AD_Org_ID) AS AD_Org_ID,
    ...
    l.PlannedAmt-fa.AmtSourceDr+fa.AmtSourceCr AS LineMargin
FROM C_ProjectLine l
    RIGHT OUTER JOIN C_ProjectIssue i ON (i.C_Project_ID=l.C_Project_ID AND i.C_ProjectIssue_ID=l.C_ProjectIssue_ID)
    LEFT OUTER JOIN Fact_Acct fa ON (fa.AD_Table_ID=623 AND fa.Record_ID=i.C_ProjectIssue_ID AND fa.M_Locator_ID IS NULL)
...
AD_Client_ID, AD_Org_ID, Created, CreatedBy, Updated, UpdatedBy, IsActive,
    DocumentNo, DateDoc, DateAcct, AD_Table_ID, Record_ID, IsSOTrx
)
AS
...
SELECT s.AD_Client_ID, s.AD_Org_ID,
    ...
    CAST((getDaysBetween(asi.GuaranteeDate,getdate())/p.GuaranteeDays)*100 as decimal(5)) AS ShelfLifeRemainingPct
FROM M_Storage s
...
CREATE VIEW temp_RV_Click_Month
AS
SELECT cc.AD_Client_ID, cc.AD_Org_ID,
    cc.Name, cc.Description, cc.TargetURL, cc.C_BPartner_ID, 
    firstOf(c.Created,'MM') AS Created,
...
SELECT il.AD_Client_ID, il.AD_Org_ID, 
	...
	CAST(i.Multiplier*LineNetAmt AS DECIMAL(15,5)) AS LineNetAmt,
	CAST(i.Multiplier*PriceList*QtyInvoiced AS DECIMAL(15,5)) AS LineListAmt,
	CASE WHEN COALESCE(il.PriceLimit, 0)=0 THEN CAST(i.Multiplier*LineNetAmt AS DECIMAL(15,5)) ELSE CAST(i.Multiplier*PriceLimit*QtyInvoiced AS DECIMAL(15,5)) END AS LineLimitAmt,
	CAST(i.Multiplier*PriceList*QtyInvoiced-LineNetAmt AS DECIMAL(15,5)) AS LineDiscountAmt,
	CASE WHEN COALESCE(il.PriceLimit,0)=0 THEN 0 ELSE CAST(i.Multiplier*LineNetAmt-PriceLimit*QtyInvoiced AS DECIMAL(15,5)) END AS LineOverLimitAmt,
	...
	AND (
		...
		OR	(o.InvoiceRule='S' AND bp.C_InvoiceSchedule_ID IS NOT NULL AND
				...
				OR	(si.InvoiceFrequency='T'
					AND ((TRUNC(o.DateOrdered) <= addDays (firstOf(getdate(),'MM'),si.InvoiceDayCutoff-1)
						AND TRUNC(getdate()) >= addDays (firstOf(o.DateOrdered,'MM'),si.InvoiceDay-1))
					OR	(TRUNC(o.DateOrdered) <= addDays (firstOf(getdate(),'MM'),si.InvoiceDayCutoff+14)
						AND TRUNC(getdate()) >= addDays (firstOf(o.DateOrdered,'MM'),si.InvoiceDay+14)))
					)
				--	Monthly
				OR	(si.InvoiceFrequency='M'
					AND TRUNC(o.DateOrdered) <= addDays (firstOf(getdate(),'MM'),si.InvoiceDayCutoff-1)	-- after cutoff
					AND TRUNC(getdate()) >= addDays (firstOf(o.DateOrdered,'MM'),si.InvoiceDay-1))		-- after invoice day
				)
			)
		)
...
; CREATE VIEW M_InOut_Candidate_v 
AS
SELECT	
	o.AD_Client_ID, o.AD_Org_ID, o.C_BPartner_ID, o.C_Order_ID,
	o.DocumentNo, o.DateOrdered, o.C_DocType_ID, 
...
DROP VIEW GL_JournalLine_Acct_v
; CREATE VIEW GL_JournalLine_Acct_v
AS
SELECT 
...
; 
DROP VIEW temp_RV_Fact_Acct_Day
; 
...
; CREATE VIEW RV_Fact_Acct_Period
AS
SELECT AD_Client_ID, AD_Org_ID,
...
; CREATE VIEW RV_C_RfQ_UnAnswered
AS
...
; CREATE VIEW RV_InOutLineConfirm
AS
...
; CREATE VIEW RV_PrintFormatDetail
AS
...
--jz derby doesn't like order by in select for create view
;


CREATE VIEW RV_BPartnerOpen
(
AD_Client_ID,AD_Org_ID, IsActive, Created,CreatedBy,Updated,UpdatedBy,
...
SELECT i.AD_Client_ID,i.AD_Org_ID, i.IsActive, i.Created,i.CreatedBy,i.Updated,i.UpdatedBy,
    ...
    COALESCE(getDaysBetween(getdate(),ips.DueDate), paymentTermDueDays(C_PaymentTerm_ID,DateInvoiced,getdate())) AS DaysDue,
    i.C_Campaign_ID, i.C_Project_ID, i.C_Activity_ID
FROM C_Invoice_v i 
...
; CREATE VIEW RV_CommissionRunDetail
AS
...
UNION   --  SalesRep
SELECT u.AD_Client_ID, u.AD_Org_ID, u.IsActive, u.Created, u.CreatedBy, u.Updated, u.UpdatedBy,
    r.R_Request_ID, u.AD_User_ID, nullif('a','a'), 
    nullif(1,1), nullif(1,1), r.R_Category_ID
FROM AD_User u
...
CREATE VIEW RV_RequestUpdates_Only
AS
SELECT MIN(AD_Client_ID) AS AD_Client_ID, MIN(AD_Org_ID) AS AD_ORG_ID, 'Y' AS IsActive, 
    CURRENT_TIMESTAMP AS Created, 0 AS CreatedBy, CURRENT_TIMESTAMP AS Updated, 0 AS UpdatedBy,
...
DROP VIEW M_InOutLineMA_vt
; CREATE VIEW M_InOutLineMA_vt
AS
SELECT m.AD_Client_ID, m.AD_Org_ID, m.IsActive, m.Created, m.CreatedBy, m.Updated, m.UpdatedBy,
    l.M_InOut_ID, m.M_InOutLine_ID, l.Line, l.M_Product_ID, 
    m.M_AttributeSetInstance_ID, m.MovementQty, l.M_Locator_ID
...
; CREATE VIEW M_MovementLineMA_v
AS
SELECT m.AD_Client_ID, m.AD_Org_ID, m.IsActive, m.Created, m.CreatedBy, m.Updated, m.UpdatedBy,
...
DROP VIEW M_MovementLineMA_vt
; CREATE VIEW M_MovementLineMA_vt
AS
SELECT m.AD_Client_ID, m.AD_Org_ID, m.IsActive, m.Created, m.CreatedBy, m.Updated, m.UpdatedBy,
    l.M_Movement_ID, m.M_MovementLine_ID, l.Line, l.M_Product_ID, 
    m.M_AttributeSetInstance_ID, m.MovementQty, l.M_Locator_ID, l.M_LocatorTo_ID
...
SELECT 'End of Views' FROM SYSIBM.SYSDUMMY1
;
