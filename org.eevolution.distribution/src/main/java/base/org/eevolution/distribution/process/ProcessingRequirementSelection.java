...
public class ProcessingRequirementSelection extends ProcessingRequirementSelectionAbstract {
    ...
    private void createDriverRequirement(MDDDriver driver, List<MDDRequirement> requirements) {
        ...
        requirements.stream()
                ...
                .forEach(requirement -> {
                    org.eevolution.distribution.model.MDDRequirementAssignment assignment = new org.eevolution.distribution.model.MDDRequirementAssignment(driver, requirement.get_ID());
    ...
    private void createVehicleRequirement(MDDVehicle vehicle, List<MDDRequirement> requirements) {
        ...
        requirements.stream()
                .filter(requirement -> requirement != null)
                .forEach(requirement -> {
                    org.eevolution.distribution.model.MDDRequirementAssignment assignment = new MDDRequirementAssignment(vehicle, requirement.get_ID());
                    assignment.setSeqNo(lineNo.updateAndGet(no -> no + 10));
                    assignment.saveEx();
                });
