...
public class VOrderReceiptIssueCP extends CPanel implements FormPanel,
		...
		ListSelectionListener, TableModelListener, ASyncProcess
{
	...
	private BorderLayout receiptLayout = new BorderLayout();
	private BorderLayout issueLayout = new BorderLayout();
	private CPanel issueOrder = new CPanel();
	private CPanel receiptOrder = new CPanel();
	...
	private JScrollPane receiptScrollPane = new JScrollPane();
	private MiniTable receipt = new MiniTable();
	...
	private void jbInit() throws Exception
	{
		...
		receiptOrder.setLayout(receiptLayout);
		issueOrder.setLayout(issueLayout);
		PanelCenter.setLayout(new java.awt.BorderLayout());
		
		PanelCenter.add(infoPanel, BorderLayout.CENTER);
		infoPanel.setOrientation(JSplitPane.VERTICAL_SPLIT);
		infoPanel.setBorder(BorderFactory.createEtchedBorder());
		infoPanel.setTopComponent(receiptOrder);
		infoPanel.setBottomComponent(issueOrder);
		infoPanel.add(receiptOrder, JSplitPane.TOP);
		infoPanel.add(issueOrder, JSplitPane.BOTTOM);
		infoPanel.setContinuousLayout(true);
		infoPanel.setPreferredSize(new Dimension(800,250));
		infoPanel.setDividerLocation(110);
		//receipt
		receiptScrollPane.setBorder(new javax.swing.border.TitledBorder("Receipt"));
		receiptScrollPane.getViewport().add(receipt, null);
		receiptOrder.add(receiptScrollPane, java.awt.BorderLayout.CENTER);
		//issue
		issueScrollPane.setBorder(new javax.swing.border.TitledBorder("Issues"));
		issueScrollPane.getViewport().add(issue, null);
		issueOrder.add(issueScrollPane, java.awt.BorderLayout.CENTER);
	...
	private void dynInit()
	{
		...
		receipt.addColumn(MPPOrder.COLUMNNAME_PP_Order_ID); //0
		//receipt.addColumn(MPPOrderBOMLine.COLUMNNAME_IsCritical); //1
		receipt.addColumn("Value"); //2
		receipt.addColumn(MPPOrderBOMLine.COLUMNNAME_M_Product_ID); //3
		receipt.addColumn(MPPOrderBOMLine.COLUMNNAME_C_UOM_ID); //4
		receipt.addColumn(MPPOrderBOMLine.COLUMNNAME_M_AttributeSetInstance_ID); //5      
		receipt.addColumn(MPPOrder.COLUMNNAME_QtyOrdered); //6   
		receipt.addColumn(MPPOrder.COLUMNNAME_QtyDelivered); //7
		receipt.addColumn("QtyToDeliver"); //8
		receipt.addColumn(MPPOrder.COLUMNNAME_QtyScrap); //9                
		receipt.addColumn("QtyOnHand"); //10               
		receipt.addColumn(MPPOrderBOMLine.COLUMNNAME_QtyReserved); //11
		receipt.addColumn("QtyAvailable"); //12
		receipt.addColumn(MLocator.COLUMNNAME_M_Locator_ID); //13       
		receipt.addColumn(MPPOrder.COLUMNNAME_M_Warehouse_ID); //14                                               
		//receipt.addColumn(MPPOrderBOMLine.COLUMNNAME_QtyBOM); //15
		//receipt.addColumn(MPPOrderBOMLine.COLUMNNAME_IsQtyPercentage); //16
		//receipt.addColumn(MPPOrderBOMLine.COLUMNNAME_QtyBatch); //17

		receipt.setMultiSelection(true);
		receipt.setRowSelectionAllowed(true);

		//  set receipt
		receipt.setColumnClass(0, IDColumn.class, false, " ");
		//receipt.setColumnClass(1, Boolean.class, true, Msg.translate(Env.getCtx(), "IsCritical"));
		receipt.setColumnClass(1, String.class, true, Msg.translate(Env.getCtx(), "Value"));
		receipt.setColumnClass(2, KeyNamePair.class, true, Msg.translate(Env.getCtx(), "M_Product_ID"));
		receipt.setColumnClass(3, KeyNamePair.class, true, Msg.translate(Env.getCtx(), "C_UOM_ID"));
		receipt.setColumnClass(4, String.class, true, Msg.translate(Env.getCtx(), "M_AttributeSetInstance_ID"));
		receipt.setColumnClass(5, BigDecimal.class, true, Msg.translate(Env.getCtx(), "QtyOrdered"));
		receipt.setColumnClass(6, BigDecimal.class, true, Msg.translate(Env.getCtx(), "QtyDelivered"));
		receipt.setColumnClass(7, BigDecimal.class, false, Msg.translate(Env.getCtx(), "QtyToDeliver"));
		receipt.setColumnClass(8, BigDecimal.class, false, Msg.translate(Env.getCtx(), "QtyScrap"));
		receipt.setColumnClass(9, BigDecimal.class, true, Msg.translate(Env.getCtx(), "QtyOnHand"));
		//receipt.setColumnClass(11, BigDecimal.class, true, Msg.translate(Env.getCtx(), "QtyReserved"));
		//receipt.setColumnClass(12, BigDecimal.class, true, Msg.translate(Env.getCtx(), "QtyAvailable"));
		receipt.setColumnClass(10, String.class, true, Msg.translate(Env.getCtx(), "M_Locator_ID"));
		receipt.setColumnClass(11, KeyNamePair.class, true, Msg.translate(Env.getCtx(), "M_Warehouse_ID"));
		//receipt.setColumnClass(15, BigDecimal.class, true, Msg.translate(Env.getCtx(), "QtyBom"));
		//receipt.setColumnClass(16, Boolean.class, true, Msg.translate(Env.getCtx(), "IsQtyPercentage"));
		//receipt.setColumnClass(17, BigDecimal.class, true, Msg.translate(Env.getCtx(), "QtyBatch"));
		
		receipt.autoSize();
		receipt.getModel().addTableModelListener(this);

		CompiereColor.setBackground(this);
		receipt.setCellSelectionEnabled(true);
		receipt.getSelectionModel().addListSelectionListener(this);
