 ...
 *****************************************************************************/
package test.functional.account;
 ...
 */
public class MInventoryPostTest extends AdempiereTestCase {
	int M_Inventory_ID=1000001; 
	...
	public void testPosting() throws Exception 
	{
			...
			if(M_Inventory_ID  > 0)
			{	
				whereClause = whereClause + MInventory.COLUMNNAME_M_Inventory_ID + "=?";
				params.add(M_Inventory_ID);
			}	
			
			Collection <MInventory> ccs =new Query(getCtx(), MInventory.Table_Name, whereClause, null )
			.setParameters(params)
			.list();
			
			for (MInventory cc : ccs)
			{
				Log.info("Cost Collector" + cc.getDocumentNo());
				MAcctSchema[] m_ass = MAcctSchema.getClientAcctSchema(Env.getCtx(), 11);
				Doc doc = Doc.get (m_ass, MInventory.Table_ID, cc.get_ID(), null);
				if (doc == null)
				{
