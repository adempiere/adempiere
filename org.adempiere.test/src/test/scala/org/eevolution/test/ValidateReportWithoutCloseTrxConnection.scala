  ...
  feature("Create a sales ticket and generate the sales order PDF form") {
    ...
    scenario("Create sales order") {
      ...
      val processInfo = ProcessBuilder.create(Context)
        ...
        .execute(trxName)
      And("The execution have not error")
      assert(!processInfo.isError)
      And("the process logs are :" + processInfo.getSummary)
      And("the process return PDF")
      assert(processInfo.getPDFReport != null)
      info(processInfo.getPDFReport.getAbsolutePath)
      And("the orignal transaction is active")
