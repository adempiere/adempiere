...
trait ProductService {
  ...
  def getProductByValue(
      value: String
  )(implicit context: Context, transaction: Transaction): Product = {
    val whereClause: StringBuilder = new StringBuilder(
      "IsStocked='Y' AND ProductType='I'"
    )
    ...
    if (value != null) {
      whereClause
        .append(" AND ")
        .append(I_M_Product.COLUMNNAME_Value)
        .append("=?")
      parameters.add(value)
    }

    val product: Product = new Query(
      context,
  ...
  def getProductCategoryByValue(
      value: String
  )(implicit context: Context, transaction: Transaction): ProductCategory = {
    val whereClause: StringBuilder = new StringBuilder()
    val parameters: util.List[Object] with Object = new util.ArrayList[Object]()
    ...
    val product: ProductCategory = new Query(
      context,
  ...
  def getUOMByCode(
      symbol: String
  )(implicit context: Context, transaction: Transaction): UOM = {
    val whereClause: StringBuilder = new StringBuilder()
    whereClause.append(I_C_UOM.COLUMNNAME_UOMSymbol).append("=?")
    val uom: UOM = new Query(
      context,
      I_C_UOM.Table_Name,
      whereClause.toString(),
      transaction.getTrxName
    )
      .setParameters(symbol)
      .first()
    uom
  }
}
