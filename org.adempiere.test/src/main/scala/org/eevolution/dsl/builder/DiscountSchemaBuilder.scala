...
object DiscountSchemaBuilder {
  ...
  def apply()(implicit context: Context , transaction: Transaction) = {
      new Builder[Mandatory, Mandatory, Optional, Mandatory, Optional , Optional , Optional , Optional, Optional]()
  }
  ...
  WithDiscountSchemaBreakTracking <: Optional
     ...
     discountSchemaBreaks : Map[DiscountSchemaBreak, Option[Integer]] = Map.empty
    )(implicit context: Context, transaction: Transaction)
  {
    ...
    def AddDiscountSchemaBreak[DiscountSchemaBreak <: WithDiscountSchemaBreakTracking : IsOnce ](discountSchemaBreak: dsl.DiscountSchemaBreak , sequenceNo: Integer): Builder[WithOrganizationTracking, WithNameTracking, WithDescriptionTracking, WithDiscountTypeTracking, WithIsPartnerFlatDiscountTracking, WithFlatDiscountTracking, WithIsQuantityBasedTracking, WithAccumulationLevelTracking, Once] =
      copy[
        ...
        WithIsQuantityBasedTracking,
        WithAccumulationLevelTracking,
        Once] (discountSchemaBreaks = discountSchemaBreaks + (discountSchemaBreak -> Some(sequenceNo)))
    ...
    AccumulationLevel  <: WithAccumulationLevelTracking : IsOnce
      ...
      val discountSchema = new DiscountSchema(context, 0, transaction.getTrxName)
