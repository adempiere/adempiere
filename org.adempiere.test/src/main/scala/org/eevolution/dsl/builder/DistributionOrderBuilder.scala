...
object DistributionOrderBuilder {
  ...
  def apply(context: Context, trxName: String) = {
    new Builder[Mandatory, Mandatory, Mandatory, Mandatory, Once, Once](
      context: Context,
      trxName: String
    )
  }
  ...
  case class Builder[
    ...
    def withDropShip[DropShipDocument <: WithDropShipTracking: IsMandatory](
        ds: Boolean
    ): Builder[
      WithOrganizationTracking,
      WithPartnerTracking,
    ...
    def AddLine[DistributionOrderLine <: WithLinesTracking: IsOnce](
        product: Product,
        quantity: Quantity
    ): Builder[
      WithOrganizationTracking,
    ...
    def build[
      ...
      if (X_C_DocType.DOCBASETYPE_DistributionOrder == baseDocumentType.get) {
        val documentTypeId = MDocType.getDocType(
