...
public class MAssetType extends X_A_Asset_Type
{
	...
	public static boolean isFixedAsset(MAsset asset) {
		if(asset.getA_Asset_Type_ID() <= 0) {
			return false;
		}
		MAssetType assetType = MAssetType.get(asset.getCtx(), asset.getA_Asset_Type_ID());
		if(assetType.getValue() == null) {
			return false;
		}
		return asset != null && A_ASSET_TYPE_MFX.equals(assetType.getValue());
	}
