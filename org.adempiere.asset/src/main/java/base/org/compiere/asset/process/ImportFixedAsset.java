 ...
 * 	@version 	$Id$
 */
public class ImportFixedAsset extends ImportFixedAssetAbstract
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		int no = 0;
		String sqlCheck = " AND AD_Client_ID=" + getAD_Client_ID();
		...
		sql = new StringBuffer ("UPDATE "+MIFixedAsset.Table_Name+ " "
			  ...
			  + " AD_Org_ID = COALESCE (AD_Org_ID,").append ("0").append ("),"
		...
		try
		{
			while (rs.hasNext()) {
				...
				try
				{
					...
					if(getDateAcct() != null)
						assetAdd.setDateAcct(getDateAcct());
					assetAdd.saveEx();
