 ...
 *         <li>BF [ 1933948 ] Correct Requisition
 */
public class MRP extends SvrProcess
{
	...
	protected void deleteMRP(int AD_Client_ID, int AD_Org_ID,int S_Resource_ID, int M_Warehouse_ID, int M_Product_ID, String trxName) throws SQLException
	{
			...
			where = getDeleteSQLWhere(MRequisitionLine.Table_Name,"mrp." + MPPMRP.COLUMNNAME_M_RequisitionLine_ID + " = " + MPPMRP.COLUMNNAME_M_RequisitionLine_ID,
					...
					+ " AND EXISTS (SELECT 1 FROM M_Requisition r WHERE r.M_Requisition_ID = M_RequisitionLine.M_Requisition_ID AND DocStatus = ?)";
			myParameters = new ArrayList(parameters);
			myParameters.add(MPPMRP.DOCSTATUS_Drafted);
			deletePO(MRequisitionLine.Table_Name, where, trxName,  myParameters.toArray());
			
			// Requisition lines are empty, now delete Requisition
			sql = new StringBuilder();
			sql.append(" NOT EXISTS (SELECT 1 FROM M_REQUISITIONLINE rl WHERE rl.M_Requisition_ID = M_REQUISITION.M_Requisition_ID )")
					.append(" AND DocStatus = ? ")
					.append(" AND AD_Client_ID = ? ");
			myParameters = new ArrayList();
			myParameters.add(MPPMRP.DOCSTATUS_Drafted);
			myParameters.add(AD_Client_ID);
			deletePO(MRequisition.Table_Name, sql.toString(), trxName,  myParameters.toArray());
			...
			if (isSynchronize()) {
				...
				myParameters = new ArrayList(parameters);
				myParameters.add(0, MPPMRP.TYPEMRP_Supply);  // Parameter expected at first position
		...
		{	
			...
			{	
				...
				{	
					...
					order.addDescription(Msg.translate(Env.getCtx(), "Generated by MRP"));
			...
			oline.setC_UOM_ID(product.getC_UOM_ID());  // UOM is taken from product
			oline.saveEx();
