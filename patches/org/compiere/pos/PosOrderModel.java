 ...
 *         *Copyright � ConSerTi
	...
	public boolean payCreditNote(MInvoice creditNote, BigDecimal amt) 
	{
		...
		payment.setAmount(getC_Currency_ID(), Env.ZERO);
		payment.setC_BankAccount_ID(m_pos.getC_BankAccount_ID());
		payment.saveEx();
		//Invoice
		MPaymentAllocate pa = new MPaymentAllocate(p_ctx, 0, null);
		pa.setC_Payment_ID(payment.getC_Payment_ID());
		pa.setC_Invoice_ID(getC_Invoice_ID());
		pa.setAmount(amt);
		pa.saveEx();
		//CreditNote
		pa = new MPaymentAllocate(p_ctx, 0, null);
		pa.setC_Payment_ID(payment.getC_Payment_ID());
	...
	public ValueNamePair[] getCreditNotes ()
	{
		try
		{
			...
			List<MInvoice> cns = new Query(p_ctx, MInvoice.Table_Name, whereClause, null)
