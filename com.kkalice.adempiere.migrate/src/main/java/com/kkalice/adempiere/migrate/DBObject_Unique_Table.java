 ...
 * Description:	detailed definition for unique constraint
 * Created:		Feb 28, 2009
 * Vendor:		K.K. Alice
...
public class DBObject_Unique_Table extends DBObjectDefinition {
	...
	/** table on which unique constraint is defined */
	private String m_table = null;
	/** whether this unique constraint is deferrable */
	private boolean m_isDeferrable = false;
	/** whether this unique constraint is initially deferred */
	 ...
	 * @param sequence
	 */
	public DBObject_Unique_Table(DBConnection parent, String name, int sequence) {
		super(parent, name, sequence);
	}

	/**
	 * initialize detailed definitions for unique constraint
	 * @param tableName the table on which the unique constraint is defined
