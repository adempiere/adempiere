 ...
 */
public class WPosPrePayment extends Window implements WPosKeyListener, EventListener {
	...
	public WPosPrePayment(WPosBasePanel posPanel, WSubOrder subOrder) {
		super();
		p_posPanel = posPanel;
	...
	public static boolean pay(WPosBasePanel posPanel, WSubOrder subOrder) {
		...
		WPosPrePayment pay = new WPosPrePayment(posPanel, subOrder);
		pay.setVisible(true);
		pay.setWidth("350px");;
		pay.setHeight("370px"); ;
	...
	public void onEvent(org.zkoss.zk.ui.event.Event event) throws Exception {
		...
		if (event.getTarget().equals(fTenderAmt) ){
			...
			if ( tender.compareTo(Env.ZERO)== 0 )
			{
				FDialog.warn(0, Msg.getMsg(p_ctx, "Tender amount zero"));
				return;
			}
			...
			if ( p_order.getDocStatus().equals(MOrder.DOCSTATUS_Completed) && 
				...
				FDialog.warn(0, Msg.getMsg(p_ctx, "Prepayment denied: Order is completed and not a Standard order"));
				return;		
			}
			else if ( p_order.getDocStatus().equals(MOrder.DOCSTATUS_Completed) && 
					p_order.getC_DocType().getDocSubTypeSO().equals(MDocType.DOCSUBTYPESO_StandardOrder) &&
					p_order.isInvoiced()) {
				FDialog.warn(0, Msg.getMsg(p_ctx, "Prepayment denied: Order is already invoiced"));
				return;		
			}
